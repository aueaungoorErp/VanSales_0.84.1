...
const sortCustomersByDistance = <T extends CustomerRouteDistanceItem>(
  customers: T[],
): T[] => {
...
export const processCustomerRouteDistances = async ({
  ...
  const distanceFromLastPosition = hasLastPosition
    ? getDistanceBetweenCoordinates(currentLocation, lastPosition)
    : null;
  const shouldReuseCachedDistances =
    canCompareDistance &&
    hasLastPosition &&
    isWithinDistanceThreshold(currentLocation, lastPosition, 15);
  ...
  if (customersToCompute.length > 0) {
    ...
    try {
      ...
      console.log(
        '[CustomerRoute] longdo unexpected error, fallback to straight line',
        {
    ...
    console.log('[CustomerRoute] distance api timing', {
      customerCount: listItems.length,
