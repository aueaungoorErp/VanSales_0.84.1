export const calculateOrderProductSummary = items => {
  ...
  if (items != null && items.length > 0) {
    items.map(item => {
      ...
      result.totalQty = Math.round((parseFloat(result.totalQty) + parseFloat(item.VTRD_QTY ? item.VTRD_QTY : 0)) * 100) / 100;
      result.totalPrice = Math.round((parseFloat(result.totalPrice) + parseFloat(item.GOODS_TOTAL_PRC ? item.GOODS_TOTAL_PRC : 0)) * 100) / 100;
      result.totalVat = Math.round((parseFloat(result.totalVat) + parseFloat(item.GOODS_VAT_TY ? item.GOODS_VAT_TY : 0)) * 100) / 100;
      result.totalDiscount = Math.round((parseFloat(result.totalDiscount) + parseFloat(item.GOODS_TOTAL_DISCOUNT ? item.GOODS_TOTAL_DISCOUNT : 0)) * 100) / 100;
      result.totalFree = Math.round((parseFloat(result.totalFree) + parseFloat(item.VTRD_Q_FREE ? item.VTRD_Q_FREE : 0)) * 100) / 100;
      result.netPrice = Math.round((parseFloat(result.netPrice) + parseFloat(item.GOODS_NET_PRC ? item.GOODS_NET_PRC : 0)) * 100) / 100;
    });
  }
...
export const calculateOrderProductProcessedSummary = (item, items) => {
  ...
  if (items) {
    result.totalPriceBeforeDiscount = items.VDI_B4_BILL_DISC ? items.VDI_B4_BILL_DISC : 0;
    ...
    result.totalPrice = items.VDI_WL === null || items.VDI_WL === 0 ? items.VDI_AF_DISC : items.VDI_AMOUNT; // VDI_B4_DISC_EX_BILL_DISC
    ...
    result.DIS_BILL_1_AFTER_DISCOUNT = items.VDI_DISC_V1 ? items.VDI_DISC_V1 : 0;
    result.DIS_BILL_2_AFTER_DISCOUNT = items.VDI_DISC_V2 ? items.VDI_DISC_V2 : 0;
    result.DIS_BILL_FINALIZE = items.VDI_AF_DISC_B4_VAT ? items.VDI_AF_DISC_B4_VAT : 0;
...
export const calculateOrderNetPriceAfterDiscount = item => {
  ...
  if (DIS_BILL_1 && netPrice && netPrice > 0) {
    DIS_BILL_1_AFTER_DISCOUNT = calculateDiscount(netPrice, DIS_BILL_1.split(/[,+]/));
    item.DIS_BILL_1_AFTER_DISCOUNT = Math.round((netPrice - DIS_BILL_1_AFTER_DISCOUNT.result) * 100) / 100;
  } else {
  ...
  if (DIS_BILL_2 && netPrice && netPrice > 0) {
    if (DIS_BILL_1_AFTER_DISCOUNT && DIS_BILL_1_AFTER_DISCOUNT.result > 0) {
      DIS_BILL_2_AFTER_DISCOUNT = calculateDiscount(item.DIS_BILL_1_AFTER_DISCOUNT, DIS_BILL_2.split(/[,+]/));
      item.DIS_BILL_2_AFTER_DISCOUNT = Math.round((item.DIS_BILL_1_AFTER_DISCOUNT - DIS_BILL_2_AFTER_DISCOUNT.result) * 100) / 100;
    } else {
      DIS_BILL_2_AFTER_DISCOUNT = calculateDiscount(netPrice, DIS_BILL_2.split(/[,+]/));
      item.DIS_BILL_2_AFTER_DISCOUNT = Math.round((netPrice - DIS_BILL_2_AFTER_DISCOUNT.result) * 100) / 100;
    }
  } else {
    item.DIS_BILL_2_AFTER_DISCOUNT = 0;
  }
  item.DIS_BILL_FINALIZE = DIS_BILL_2_AFTER_DISCOUNT ? item.DIS_BILL_2_AFTER_DISCOUNT : item.DIS_BILL_1_AFTER_DISCOUNT;
  return item;
};
  ...
  if (DIS_COUNT_TYPE1 && !DIS_COUNT_TYPE2) {
    ...
    if (DIS_BILL_2 && totalPrice && totalPrice > 0) {
      if (DIS_BILL_1_AFTER_DISCOUNT && DIS_BILL_1_AFTER_DISCOUNT > 0) {
        DIS_BILL_2_AFTER_DISCOUNT = parseFloat(DIS_BILL_2) / 100 * (totalPrice - DIS_BILL_1 / 100 * totalPrice);
  ...
  for (var i = 0; i < discountInput.length; i++) {
    ...
    if (discount) {
      if (discount.slice(-1).match(/^(b|B|บ)$/)) {
        var iDiscount = discount.substring(0, discount.lastIndexOf(discount.slice(-1)));
        if (!isNaN(iDiscount)) {
        ...
        var iDiscount = discount.substring(0, discount.lastIndexOf(discount.slice(-3)));
        if (!isNaN(iDiscount)) {
          totalDiscount = parseFloat(totalDiscount) + parseFloat(iDiscount);
          uPrcRemain = uPrcRemain - iDiscount;
        } else {
        ...
        var iDiscount = discount.slice(-1) === '%' ? discount.substring(0, discount.lastIndexOf('%')) : discount;
        if (!isNaN(iDiscount)) {
...
export const calculateOrderProductTotalDiscount = (uPrc, discountInput, qty) => {
  const result = calculateDiscount(uPrc, discountInput);
  return {
    result: !result.errorMessage ? Math.round(qty * result.result * 100) / 100 : 0,
