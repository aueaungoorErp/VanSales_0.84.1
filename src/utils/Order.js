...
export const generateHeaderForUpdate = (customer, header, orderType, orderSubType) => {
  result = {
    ...header,
    VDI_KEY: customer.AR_KEY,
    AR_NAME: customer.AR_NAME,
...
export const convertProductItemLastBillToOrderItem = items => {
  return items.map((item, key) => {
    return result = {
      ...
      //
      VTRD_CODE: item.GOODS_CODE,
      // รหัสสินค้า หน้าแสดงรายการ
      VTRD_NAMES: item.SKU_NAME,
      // ชื่อสินค้า หน้าแสดงรายการ
      VTRD_QTY: item.TRD_QTY,
      //  จำนวน หน้าแสดงรายการ
      VTRD_U_PRC: item.TRD_U_PRC,
      //
      VTRD_U_PRC_KEYIN: item.TRD_U_PRC,
      //ราคาต่อหน่วย หน้าแสดงรายการ
      ...
      //
      VTRD_TDSC_V: item.TRD_DSC_KEYIN,
      // ส่วนลด หน้าแสดงรายการ
      ...
      GOODS_TOTAL_PRC: item.TRD_G_AMT,
      //ราคารวม หน้าแสดงรายการ
...
export const convertProductItemFromServerProcessV3 = item => {
  result = {
    ...
    TRD_Q_FREE: item.GOODS_FREE ? parseFloat(item.GOODS_FREE).toFixed(2) : '' /* จำนวนแถม*/,
    TRD_K_U_PRC: parseFloat(item.ARPLU_U_PRC).toFixed(3) /* ราคาที่ใช้ขายในบิลนี้*/,
    TRD_DSC_KEYIN: item.GOODS_DISCOUNT ? item.GOODS_DISCOUNT.includes('B') && item.GOODS_DISCOUNT.includes('*') ? item.GOODS_DISCOUNT != parseFloat(item.GOODS_DISCOUNT.split('*')[0]) + '*' + item.GOODS_QTY + 'B' ? parseFloat(item.GOODS_DISCOUNT.split('*')[0]) + '*' + item.GOODS_QTY + 'B' : item.GOODS_DISCOUNT : parseFloat(item.GOODS_DISCOUNT) + '*' + item.GOODS_QTY + 'B' : '0' /* ส่วนลดตามที่บันทึกจากจอภาพ*/,
...
export const convertProductItemFromServerProcess2V3 = item => {
  result = {
    ...
    TRD_DSC_KEYIN: item.VTRD_U_DISC_TEXT ? parseFloat(item.VTRD_U_DISC_TEXT) + '*' + item.VTRD_QTY + 'B' : parseFloat(item.VTRD_U_DSC) + '*' + item.VTRD_QTY + 'B',
    TRD_K_U_PRC: parseFloat(item.VTRD_U_PRC_KEYIN).toFixed(2),
    TRD_WL: item.FROMWL_CODE ? item.FROMWL_CODE : '' /* ตำแหน่งเก็บที่เก็บสินค้า*/,
    TRD_TO_WL: item.TOWL_CODE ? item.TOWL_CODE : '' /* เฉพาะกรณีย้ายตำแหน่งเก็บ ให้ทำการระบุคลังที่รับย้าย*/,
    TRD_OPTION: '',
    TRD_U_VATIO: '2' /* 0 ไม่ได้ใช้ 1 ยังไม่รวมภพ 2 รวมภพแล้ว*/,
    X_MODEL: 1
  };
...
export const convertProductItemFromServerProcess = (product, item, _round) => {
  ...
  switch (_round) {
    ...
    case 2:
      //  ปัดขึ้นลงเป็นสลึง
      ret = bath + (satang * 100 > 1 && satang * 100 <= 12 ? 0 : satang * 100 > 13 && satang * 100 <= 25 ? 25 : satang * 100 > 26 && satang * 100 <= 37 ? 25 : satang * 100 > 38 && satang * 100 <= 50 ? 50 : satang * 100 > 51 && satang * 100 <= 62 ? 50 : satang * 100 > 63 && satang * 100 <= 75 ? 75 : satang * 100 > 76 && satang * 100 <= 87 ? 75 : satang * 100 > 88 && satang * 100 <= 100 ? 100 : 100) / 100;
      break;
    case 11:
      //  ปัดขึ้นเป็นบาท
    ...
    case 12:
      //  ปัดขึ้นเป็นสลึง
      ret = bath + (satang * 100 > 1 && satang * 100 <= 25 ? 25 : satang * 100 > 25 && satang * 100 <= 50 ? 50 : satang * 100 > 50 && satang * 100 <= 75 ? 75 : 100) / 100;
      break;
    case 21:
      //  ปัดลงเป็นบาท
    ...
    case 22:
      //  ปัดลงเป็นสลึง
      ret = bath + (satang * 100 > 75 && satang * 100 <= 100 ? 100 : satang * 100 > 50 && satang * 100 <= 75 ? 75 : satang * 100 > 25 && satang * 100 <= 50 ? 50 : 0) / 100;
      break;
  }
  //Bazz ปัดเศษสะดวกทอน
...
export const genenrateOrderForProcessToServer = (item, ignoreCpgn, processType, check, processTypeDetail) => {
  const {
    header,
    orderProductSummary,
    productListItems
  ...
  if (!check) {
    result = {
      ...
      DIS_BILL_1: orderProductSummary.DIS_BILL_1 ? orderProductSummary.DIS_BILL_1 !== '' ? orderProductSummary.DIS_BILL_1 + 'B' : orderProductSummary.DIS_BILL_1 : '',
      DIS_BILL_2: orderProductSummary.DIS_BILL_2 ? orderProductSummary.DIS_BILL_2 : '',
...
export const genenrateOrderForProcessToServerSCR = (order, items, ignoreCpgn, processType) => {
  const {
    header,
    orderProductSummary
  } = order;
...
export const genenrateOrderForCreateToServer = (item, mile, position, dscfTxnId) => {
  let {
    headerProcessed,
...
export const generateResponseFromServer = obj => {
  ...
  if (obj.AROE) {
    newAR = {
      ...
      AROE_G_KEYIN: obj.AROE.AROE_G_KEYIN ? parseFloat(obj.AROE.AROE_G_KEYIN) : null,
      ...
      AROE_TDSC_KEYINV: obj.AROE.AROE_TDSC_KEYINV ? parseFloat(obj.AROE.AROE_TDSC_KEYINV) : null,
      AROE_WH_TAX: obj.AROE.AROE_WH_TAX ? parseFloat(obj.AROE.AROE_WH_TAX) : null,
      AROE_XCHG: obj.AROE.AROE_XCHG ? parseFloat(obj.AROE.AROE_XCHG) : null
    };
  } else {
    newAR = {
      ...obj.ARDETAIL,
      ARDETAIL_ARCD: obj.ARDETAIL.ARDETAIL_ARCD ? parseFloat(obj.ARDETAIL.ARDETAIL_ARCD) : null,
      ARDETAIL_A_AMT: obj.ARDETAIL.ARDETAIL_A_AMT ? parseFloat(obj.ARDETAIL.ARDETAIL_A_AMT) : null,
      ARDETAIL_A_SNV: obj.ARDETAIL.ARDETAIL_A_SNV ? parseFloat(obj.ARDETAIL.ARDETAIL_A_SNV) : null,
      ARDETAIL_A_SV: obj.ARDETAIL.ARDETAIL_A_SV ? parseFloat(obj.ARDETAIL.ARDETAIL_A_SV) : null,
      ARDETAIL_A_VAT: obj.ARDETAIL.ARDETAIL_A_VAT ? parseFloat(obj.ARDETAIL.ARDETAIL_A_VAT) : null,
      ARDETAIL_B_AMT: obj.ARDETAIL.ARDETAIL_B_AMT ? parseFloat(obj.ARDETAIL.ARDETAIL_B_AMT) : null,
      ARDETAIL_B_SNV: obj.ARDETAIL.ARDETAIL_B_SNV ? parseFloat(obj.ARDETAIL.ARDETAIL_B_SNV) : null,
      ARDETAIL_B_SV: obj.ARDETAIL.ARDETAIL_B_SV ? parseFloat(obj.ARDETAIL.ARDETAIL_B_SV) : null,
      ARDETAIL_B_VAT: obj.ARDETAIL.ARDETAIL_B_VAT ? parseFloat(obj.ARDETAIL.ARDETAIL_B_VAT) : null,
      ARDETAIL_G_KEYIN: obj.ARDETAIL.ARDETAIL_G_KEYIN ? parseFloat(obj.ARDETAIL.ARDETAIL_G_KEYIN) : null,
      ARDETAIL_G_SNV: obj.ARDETAIL.ARDETAIL_G_SNV ? parseFloat(obj.ARDETAIL.ARDETAIL_G_SNV) : null,
      ARDETAIL_G_SV: obj.ARDETAIL.ARDETAIL_G_SV ? parseFloat(obj.ARDETAIL.ARDETAIL_G_SV) : null,
      ARDETAIL_G_VAT: obj.ARDETAIL.ARDETAIL_G_VAT ? parseFloat(obj.ARDETAIL.ARDETAIL_G_VAT) : null,
      ARDETAIL_N_AMT: obj.ARDETAIL.ARDETAIL_N_AMT ? parseFloat(obj.ARDETAIL.ARDETAIL_N_AMT) : null,
      ARDETAIL_N_SNV: obj.ARDETAIL.ARDETAIL_N_SNV ? parseFloat(obj.ARDETAIL.ARDETAIL_N_SNV) : null,
      ARDETAIL_N_SV: obj.ARDETAIL.ARDETAIL_N_SV ? parseFloat(obj.ARDETAIL.ARDETAIL_N_SV) : null,
      ARDETAIL_N_VAT: obj.ARDETAIL.ARDETAIL_N_VAT ? parseFloat(obj.ARDETAIL.ARDETAIL_N_VAT) : null,
      ARDETAIL_TDSC_KEYINV: obj.ARDETAIL.ARDETAIL_TDSC_KEYINV ? parseFloat(obj.ARDETAIL.ARDETAIL_TDSC_KEYINV) : null,
      ARDETAIL_WH_TAX: obj.ARDETAIL.ARDETAIL_WH_TAX ? parseFloat(obj.ARDETAIL.ARDETAIL_WH_TAX) : null,
      ARDETAIL_XCHG: obj.ARDETAIL.ARDETAIL_XCHG ? parseFloat(obj.ARDETAIL.ARDETAIL_XCHG) : null
    };
  }
  let newDocInfo = {
    ...obj.DOCINFO,
    ...
    DI_GPS_LAT_S: obj.DOCINFO.DI_GPS_LAT_S ? parseFloat(obj.DOCINFO.DI_GPS_LAT_S) : null,
    DI_GPS_LONG_S: obj.DOCINFO.DI_GPS_LONG_S ? parseFloat(obj.DOCINFO.DI_GPS_LONG_S) : null
  };
  let newTRANSTKD = [];
  ...
  return {
    ...
    AROE: newAR,
    DOCINFO: newDocInfo,
    TRANSTKD: newTRANSTKD,
    TRANPAYD: obj.paymentMethod
  };
};
...
export const genenrateMultipleAttachImageToServer = (vdiKey, vdiAr, imgFiles) => {
  var bodyFormData = new FormData();
  bodyFormData.append('VDI_KEY', vdiKey);
  bodyFormData.append('VDI_AR', vdiAr);
...
export const generateHeaderOutStandingCreatePayment = (header, item) => {
  ...
  for (var i = 0; i < item.cheques.length; i++) {
    ...
    if (cheque.checked) {
      cheques = {
        ...
        [`VPH_CHEQUE${i + 1}_DATE`]: moment(cheque.chequeDate, 'DD/MM/YYYY').add(1, 'days').toJSON(),
  ...
  return {
    ...
    VPH_TRANSFER_QR_AMT: item.qrcode.checked && item.qrcode.pay ? item.qrcode.pay : 0,
    VPH_TRANSFER_BANK: item.transfer.checked && item.transfer.bankAccountItem ? item.transfer.bankAccountItem : 0,
    VPH_TRANSFER_AMT: item.transfer.checked && item.transfer.pay ? item.transfer.pay : 0,
