...
import { ORDER_TYPE_BOOKING, ORDER_TYPE_QUOTATION } from '../constant/orderTypes';
import { fetchDocumentSwitchSettings, parseDocumentSwitchResponse } from '../api/LookupErpServices/lookup-erp-services';
...
export const shouldLoadDefaultBookingExpiryDates = arOrderType => arOrderType === ORDER_TYPE_BOOKING || arOrderType === ORDER_TYPE_QUOTATION || arOrderType === 'เสนอราคา';
...
export const extractDocumentExpiryDaysFromSettings = responseData => {
  ...
  const oeDays = readDocumentSwitchField(firstRow, DOC_SWITCH_FIELD_OE) ?? readDocumentSwitchField(responseData, DOC_SWITCH_FIELD_OE);
  const poDays = readDocumentSwitchField(firstRow, DOC_SWITCH_FIELD_PO) ?? readDocumentSwitchField(responseData, DOC_SWITCH_FIELD_PO);
...
export const getExpiryDaysForOrderType = (arOrderType, parsedDays) => {
  const usesOeAge = arOrderType === ORDER_TYPE_BOOKING || arOrderType === ORDER_TYPE_QUOTATION || arOrderType === 'เสนอราคา';
  if (usesOeAge) {
    return {
...
export const buildBookingExpiryDatesFromToday = days => {
  const safeDays = Number.isFinite(Number(days)) && Number(days) >= 0 ? Number(days) : FALLBACK_DOC_EXPIRY_DAYS;
...
export const loadDefaultBookingExpiryDates = async arOrderType => {
  try {
    ...
    const {
      usedField,
      usedDays
    } = getExpiryDaysForOrderType(arOrderType, parsedDays);
