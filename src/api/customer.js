...
export const customerSearchListApi = criteria => {
  return new Promise((resolve, reject) => {
    Request.instance.get(`/Customer?${criteria}`).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const customerSearchListV3Api = async (arCode, arKey) => {
  ...
  const arCat = arKey === undefined ? '' : arKey?.ARCAT_NAME == 'ทั้งหมด' || arKey?.ARCAT_NAME == null ? '' : arKey?.ARCAT_NAME;
...
export const customerSearchArLineListV3Api = async criteria => {
  ...
  const KEYWORD = keywords.length > 0 ? `AND (AR_CODE LIKE '%${pattern}%' OR AR_NAME LIKE '%${pattern}%') AND VANCNF_MACHINE = '${VANCONFIG.VANCNF_MACHINE}'` : `AND VANCNF_MACHINE = '${VANCONFIG.VANCNF_MACHINE}'`;
  const ARCAT_KEY = typeof criteria.ARCAT_KEY == 'object' ? '' : criteria.ARCAT_KEY ? "and AR_ARCAT = '" + criteria.ARCAT_KEY + "'" : '';
...
export const findCustomerByIdApi = id => {
  return new Promise((resolve, reject) => {
    Request.instance.get(`/Customer/${id}`).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const createTempCusApi = data => {
  return new Promise((resolve, reject) => {
    Request.instance.post(`/Customer/Create/TempCustomer`, data).then(v => {
      resolve(v.data);
    }).catch(error => {
      reject(error);
    });
...
export const executiveV3Api = data => {
  return new Promise((resolve, reject) => {
    Request.instanceV3.post(`/Executive`, data).then(v => {
      resolve(v.data);
    }).catch(error => {
      reject(error);
    });
...
export const searchCustomerNearByApi = criteria => {
  return new Promise((resolve, reject) => {
    Request.instance.get(`/Customer/nearby/${criteria.C_LAT}/${criteria.C_LNT}/${criteria.radius}`).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const closeCustomerAccountApi = data => {
  return new Promise((resolve, reject) => {
    Request.instance.post(`/Customer/State/Close`, data).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const customerSkipApi = id => {
  return new Promise((resolve, reject) => {
    Request.instance.get(`/Customer/Skip/${id}`).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const NewArFileV3Api = bodyRequest => {
  return new Promise((resolve, reject) => {
    Request.instanceV3.post('/CreateUpdateMaster', bodyRequest).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
      ...
      if (y.ResponseCode == 200 && parseInt(responseData2.RECORD_COUNT) > 0) {
        ...
        await updateErpV3Api(bodyRequest3).then(z => {
          ...
          if (z.ResponseCode == 200 && parseInt(responseData3.RECORD_COUNT) > 0) {
            ARPRB_KEY = responseData3.ARPRB_KEY == '' ? 0 : responseData3.ARPRB_KEY;
          }
        });
      }
    });
