...
export const requestBBLQrCodeApi = ({ baseUrl, taxIdNo, amount, extra1 = '' }) => {
    return new Promise((resolve, reject) => {
        const normalizedBaseUrl = normalizePaymentBaseUrl(baseUrl)
        ...
        .then(v => {
            console.log('[BBL QR][response]', v.data)
            resolve(v.data)
        }).catch((err) => {
            console.log('[BBL QR][error]', {
                message: err?.message,
...
export const requestBBLPaymentInquiryApi = ({
    ...
    return new Promise((resolve, reject) => {
        ...
        axios.post(
            `${normalizedBaseUrl}/api/v1/payments/bbl/payment-inquiry`,
            requestBodyRaw,
            {
                timeout: appConfig.REQUEST_TIMEOUT_MS,
                headers: {
