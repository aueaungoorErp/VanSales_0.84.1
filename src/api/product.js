...
const logLookupResponseSummary = (label, response) => {
  try {
    ...
    const parsedResponseData = payload?.ResponseData ? JSON.parse(payload.ResponseData) : null;
    const dataKey = parsedResponseData ? Object.keys(parsedResponseData).find(key => Array.isArray(parsedResponseData[key]) && !['RECORD_COUNT', 'OFFSET', 'FETCH'].includes(key)) : null;
...
export const productSearchListApi = criteria => {
  return new Promise((resolve, reject) => {
    Request.instance.get(`/Goods?${criteria}`).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const productSkuSearchListApi = criteria => {
  return new Promise((resolve, reject) => {
    Request.instance.get(`/Sku?${criteria}`).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const processOrderItemApi = (id, data) => {
  return new Promise((resolve, reject) => {
    Request.instance.post(`/Order/Process/Item/${id}`, data).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const processOrderItemV3Api = async data => {
  return new Promise((resolve, reject) => {
    Request.instanceV3.post('/UpdateErp', data).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const processOrderTransferApi = (id, data) => {
  return new Promise((resolve, reject) => {
    Request.instance.post(`/Order/Process/Item/Transfer/${id}`, data).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const productSearchByGoodCodeApi = (code, ARCODE, isTransfer) => {
  return new Promise((resolve, reject) => {
    Request.instance.get(`/Goods/${code}?ARCODE=${ARCODE}&isTransfer=${isTransfer}`).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const productSkuAltSearchListApi = criteria => {
  return new Promise((resolve, reject) => {
    Request.instance.get(`/Goods/GetGoodSkuAltItems/?${criteria}`).then(v => {
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
  ...
  if (VANCONFIG.VANCNF_ARPRB_MODE == 0 || VANCONFIG.VANCNF_ARPRB_MODE == 1 && AR_KEY === undefined) {
    ...
    await readErpV3Api(bodyRequest2).then(async z => {
      ...
      if (z.ResponseCode == 200 && parseInt(responseData2.RECORD_COUNT) > 0) {
        ARPRB_CODE = responseData2.READARPRBCODEBYARPRBKEY[0].ARPRB_CODE == '' ? 0 : responseData2.READARPRBCODEBYARPRBKEY[0].ARPRB_CODE;
      }
    });
    ...
    await readErpV3Api(bodyRequest2).then(async y => {
      ...
      if (y.ResponseCode == 200 && parseInt(responseData2.RECORD_COUNT) > 0) {
        ...
        await updateErpV3Api(bodyRequest3).then(z => {
          ...
          if (z.ResponseCode == 200 && parseInt(responseData3.RECORD_COUNT) > 0) {
            ARPRB_CODE = responseData3.ARPRB_CODE == '' ? 0 : responseData3.ARPRB_CODE;
          }
        });
      }
    });
...
export const productSearchListV3Api = async (GOODS_CODE, ARPRB_CODE, criteria) => {
  const LoginGUID = await getLoginGuID();
  //const KEYWORD = "and SKU_CODE = '" + skuCode + "' ";
  const LIMIT = JSON.stringify(criteria.LIMIT);
  const OFFSET = 0; //JSON.stringify(criteria.OFFSET);

  const ICDEPT_THAIDESC = criteria.ICDEPT_THAIDESC ? "and ICCAT_NAME = '" + criteria.ICDEPT_THAIDESC + "'" : '';
  ...
  const KEYWORD = criteria.KEYWORD ? ' and (' + criteria.KEYWORD.trim().split(/\s+/).map(word => ` (SKU_NAME like '%${word}%' OR GOODS_CODE like '%${word}%')`).join(' AND ') + ')' : '';
  const bodyRequest = {
    'BPAPUS-BPAPSV': appConfig.BPAPUS_BPAPSV,
    'BPAPUS-LOGIN-GUID': LoginGUID,
...
export const productSearchListV3Api_bk = async (GOODS_CODE, ARPRB_CODE, criteria) => {
  const LoginGUID = await getLoginGuID();
  //const KEYWORD = "and SKU_CODE = '" + skuCode + "' ";
  const LIMIT = JSON.stringify(criteria.LIMIT);
  const OFFSET = 0; //JSON.stringify(criteria.OFFSET);
...
export const productSearchShowRepack = async (GOODS_CODE, ARPRB_CODE, criteria) => {
  const LoginGUID = await getLoginGuID();
  //const KEYWORD = "and SKU_CODE = '" + skuCode + "' ";
  const LIMIT = JSON.stringify(criteria.LIMIT);
...
export const productSearchListByVanV3Api = async criteria => {
  ...
  const ICDEPT_KEY = criteria.ICDEPT_KEY ? "and SKU_ICCAT = '" + criteria.ICDEPT_KEY + "'" : '';
  // const KEYWORD = criteria.KEYWORD
  //   ? "and (SKU_CODE like '%" +
  ...
  const KEYWORD = criteria.KEYWORD ? ' and (' + criteria.KEYWORD.trim().split(/\s+/).map(word => ` (SKU_CODE like '%${word}%' OR SKU_NAME like '%${word}%' OR GOODS_CODE like '%${word}%')`).join(' AND ') + ')' + " and VANCNF_MACHINE = '" + VANCONFIG.VANCNF_MACHINE + "'" : "and VANCNF_MACHINE = '" + VANCONFIG.VANCNF_MACHINE + "'";
...
export const productSearchListByARPLUAPiBykey = async (ARPLU_ARPRB, criteria) => {
  ...
  const KEYWORD = criteria.KEYWORD ? ' and (' + criteria.KEYWORD.trim().split(/\s+/).map(word => ` (SKU_CODE like '%${word}%' OR SKU_NAME like '%${word}%' OR GOODS_CODE like '%${word}%')`).join(' AND ') + ')' : '';
  let LIMIT = JSON.stringify(criteria.LIMIT);
  let OFFSET = JSON.stringify(criteria.OFFSET);
  const bodyRequest = {
    'BPAPUS-BPAPSV': appConfig.BPAPUS_BPAPSV,
  ...
  logLookupRequestSummary('productSearchListByARPLUAPiBykey REQUEST', bodyRequest, {
  ...
  return new Promise((resolve, reject) => {
    Request.instanceV3.post('/LookupErp', bodyRequest).then(v => {
      logLookupResponseSummary('productSearchListByARPLUAPiBykey RESPONSE', v);
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
// 89.หาราคาขายสินค้าในตารางราคา (Sp000221)
export const productSearchListByARPLUAPiByCode = async (ARPRB_CODE, WL_CODE, criteria, allowNegativeStock = false) => {
  const LoginGUID = await getLoginGuID();
  const {
    VANCONFIG
  } = await getUserToken();
  ...
  const bodyRequest = {
    ...
    'BPAPUS-FUNCTION': 'Sp000221',
    'BPAPUS-PARAM': '',
    'BPAPUS-FILTER': "AND (ARPRB_CODE  = '" + ARPRB_CODE + "') AND SKU_KEY IN (SELECT SKM_SKU FROM SKUMOVE WHERE SKM_WL= '" + WL_CODE + "'  GROUP BY SKM_SKU" + (allowNegativeStock ? '' : ' HAVING SUM(SKM_QTY)>0') + ') ' + KEYWORD + ICDEPT_KEY,
    'BPAPUS-ORDERBY': '',
    'BPAPUS-OFFSET': OFFSET,
    'BPAPUS-FETCH': LIMIT
  };
  logLookupRequestSummary('productSearchListByARPLUAPiByCode REQUEST', bodyRequest, {
  ...
  return new Promise((resolve, reject) => {
    Request.instanceV3.post('/LookupErp', bodyRequest).then(v => {
      logLookupResponseSummary('productSearchListByARPLUAPiByCode RESPONSE', v);
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const productSearchListByARPLUAPiByCodeAndWHIS1 = async (ARPRB_CODE, WL_CODE, criteria) => {
  ...
  let LIMIT = JSON.stringify(criteria.LIMIT);
  let OFFSET = JSON.stringify(criteria.OFFSET);
  let filter = ' AND ( ' + " (ARPRB_CODE  = '" + ARPRB_CODE + "') AND " + ' SKU_KEY IN (SELECT SKM_SKU FROM SKUMOVE WHERE SKM_WL IN (' + WL_CODE.map(code => `'${code}'`).join(', ') + ')  GROUP BY SKM_SKU HAVING SUM(SKM_QTY)>0)) ' + KEYWORD + ICDEPT_KEY;
  const bodyRequest = {
    'BPAPUS-BPAPSV': appConfig.BPAPUS_BPAPSV,
    'BPAPUS-LOGIN-GUID': LoginGUID,
    'BPAPUS-FUNCTION': 'Sp000221',
  ...
  logLookupRequestSummary('productSearchListByARPLUAPiByCodeAndWHIS1 REQUEST', bodyRequest, {
    arprbCode: ARPRB_CODE,
  ...
  return new Promise((resolve, reject) => {
    Request.instanceV3.post('/LookupErp', bodyRequest).then(v => {
      logLookupResponseSummary('productSearchListByARPLUAPiByCodeAndWHIS1 RESPONSE', v);
      resolve(v.data);
    }).catch(err => {
      reject(err);
    });
...
export const productSearchOtherListByVanV3Api_BK1 = async criteria => {
  ...
  const KEYWORD = criteria.KEYWORD ? ' and (' + criteria.KEYWORD.trim().split(/\s+/).map(word => ` (SKU_CODE like '%${word}%' OR SKU_NAME like '%${word}%' OR GOODS_CODE like '%${word}%')`).join(' AND ') + ')' + " and VANCNF_MACHINE = '" + settingConfig.vanCNFMachine + "'" : "and VANCNF_MACHINE = '" + settingConfig.vanCNFMachine + "'";
  const LIMIT = JSON.stringify(criteria.LIMIT);
  const OFFSET = JSON.stringify(criteria.OFFSET);
  ...
  return new Promise((resolve, reject) => {
    Request.instanceV3.post('/UpdateErp', bodyRequest).then(v => {
      const {
        ReasonString,
        ResponseCode,
        ResponseData
      ...
      if (ResponseCode == 200) {
        ...
        for (let i in TRANSTKD) {
          if (settingConfig.VANCONFIG.VANCNF_SKU_LIMIT == '4' && TRANSTKD[i].TRD_TO_WL == settingConfig.VANCONFIG.VANCNF_WL || settingConfig.VANCONFIG.VANCNF_SKU_LIMIT == '3' && TRANSTKD[i].TRD_WL == settingConfig.VANCONFIG.VANCNF_WL) {
            let newObj = {
              VANCNF_KEY: settingConfig.VANCONFIG.VANCNF_KEY,
              VANCNF_MACHINE: settingConfig.VANCONFIG.VANCNF_MACHINE,
              VANBRN_KEY: '',
              BRN_KEY: '',
              //parseFloat(TRANSTKD[i].TRD_Q_FREE).toFixed(2),
              BRN_CODE: '',
              //TRANSTKD[i].TRD_OPTION,
              BRN_NAME: '',
              //TRANSTKD[i].TRD_KEYIN,
              SKU_KEY: '',
              //TRANSTKD[i].TRD_NAMES ? TRANSTKD[i].TRD_NAMES : null,
              SKU_CODE: TRANSTKD[i].SKU_BARCODE,
              SKU_NAME: TRANSTKD[i].SKU_NAME,
              //    parseFloat(TRANSTKD[i].TRD_QTY).toFixed(2),
              GOODS_KEY: '',
              //TRANSTKD[i].GOODS_CODE,
            ...
            if (criteria.KEYWORD == null) {
              ...
              if (TRANSTKD[i].SKU_NAME.includes(criteria.KEYWORD.trim()) || TRANSTKD[i].SKU_CODE.includes(criteria.KEYWORD.trim()) || TRANSTKD[i].SKU_BARCODE.includes(criteria.KEYWORD.trim())) {
                ImpTrhDetail.push(newObj);
              }
            }
          }
