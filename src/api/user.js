...
import { getUserToken, getLoginGuID, getWSv3SettingConfig, getDeviceUniqeId, unRegisterApi } from '../utils/Token';
...
export const loginApi = async userLogin => {
  ...
  return new Promise((resolve, reject) => {
    if (VANCONFIG && VANCONFIG.VANCNF_MACHINE) {
      ...
      Request.instance.post('/Login', userLogin).then(v => {
        const {
          RESULT_DATA,
          STATUS
        } = v.data;
        if (STATUS === '00') {
          Request.setHeaders({
            userToken: RESULT_DATA.USER_TOKEN
          });
        }
        resolve(v.data);
      }).catch(err => {
...
export const registerV3Api = async (username, password) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (uniqueId) {
      ...
      await Request.instanceV3.post('/DevUsers', bodyRequest1).then(async m => {
        const {
          ResponseCode1
        } = m.data;
      ...
      await Request.instanceV3.post('/DevUsers', bodyRequest).then(async v => {
        ...
        if (ResponseCode == 200) {
          const bodyRequest = {
            ...
            'BPAPUS-PARAM': '{   "BPAPUS-MACHINE": ' + JSON.stringify(uniqueId) + ',   "BPAPUS-USERID": "' + username + '",   "BPAPUS-PASSWORD": "' + password + '"}'
          };
