...
import { BPAPUS_FUNCTION_BK_CODE, GET_SELL_ORDER_DOC_INFO, GET_INVOICE_DOC_INFO, BPAPUS_FUNCTION_DM_CODE, GET_PRINT_REPORT, GET_REPORT_NAME, GET_PRINT_STATUS, GET_RECEIVE_DOC_INFO, GET_OTHER_DOC_INFO, BPAPUS_FUNCTION_V_CODE } from '../constant/bPlusApi';
...
export const LookupErpBookAPi = (vanCNF, fromDate, toDate) => {
  return new Promise(async (resolve, reject) => {
    ...
    const RequestBody = {
      ...
      'BPAPUS-PARAM': '',
      'BPAPUS-FILTER': "and CONVERT(DATETIME, DI_DATE)  BETWEEN  CONVERT(DATETIME,'" + fromDate + "') and (CONVERT(DATETIME,'" + toDate + "' ))" + "and (DT_KEY = '" + userToken.VANCONFIG?.VANCNF_CASHSALES_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_INV_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_CASHRTN_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_RTN_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_BOOK_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_TRANSFER_DT + "' or DT_KEY = '" +
    ...
    lookupErpV3Api(RequestBody).then(v => {
      const {
        ReasonString,
        ResponseCode,
        ResponseData
    ...
    try {
      ...
      switch (uri) {
        ...
        case "SalesOrderByArline":
          retDT = " and (DT_KEY = '" + userToken.VANCONFIG?.VANCNF_CASHSALES_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_INV_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_CASHRTN_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_RTN_DT + "')";
          break;
        case "SalesOrderByDocType":
        case "DocumentItemsDetails":
        case "DocumentItems":
          retDT = " and (DT_KEY = '" + userToken.VANCONFIG?.VANCNF_CASHSALES_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_INV_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_CASHRTN_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_RTN_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_BOOK_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_QUOTE_DT + "')";
          break;
        default:
          retDT = " and (DT_KEY = '" + userToken.VANCONFIG?.VANCNF_CASHSALES_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_INV_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_CASHRTN_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_RTN_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_BOOK_DT + "' or DT_KEY = '" + userToken.VANCONFIG?.VANCNF_QUOTE_DT + "')";
          break;
      ...
      const RequestBody = {
        ...
        'BPAPUS-PARAM': '',
        'BPAPUS-FILTER': "and CONVERT(DATETIME, DI_DATE)  BETWEEN  CONVERT(DATETIME,'" + fromDate + "') and (CONVERT(DATETIME,'" + toDate + "' ))" + retDT,
        'BPAPUS-ORDERBY': 'order by DI_DATE desc',
        'BPAPUS-OFFSET': '0',
        'BPAPUS-FETCH': '0'
      };
...
export const UpdateErpGetSellOrderDocInfoAPi = DI_KEY => {
  return new Promise(async (resolve, reject) => {
    ...
    updateErpV3Api(RequestBody).then(v => {
      const {
        ReasonString,
        ResponseCode,
        ResponseData
...
export const GetReportNameApi = () => {
  return new Promise(async (resolve, reject) => {
    ...
    RptServerErpV3Api(RequestBody).then(v => {
      const {
        ReasonString,
        ResponseCode,
        ResponseData
...
export const GetPrintReportApi = data => {
  return new Promise(async (resolve, reject) => {
    ...
    const RequestBody = {
      ...
      'BPAPUS-PARAM': '{\r\n    "RPTSVR_GRANT": "' + RPTSVR_GRANT + '",\r\n    "RPTSVR_GUID": "' + data.RPTSVR_GUID + '",\r\n    "RPTQUE_RQST_FROMDATE": "' + data.FROMDATE + '",\r\n    "RPTQUE_RQST_TODATE": "' + data.TODATE + '",\r\n    "RPTQUE_RQST_OPTN": "",\r\n    "RPTQUE_RQST_PARAM": ""\r\n}',
      'BPAPUS-FILTER': '',
      'BPAPUS-ORDERBY': '',
      'BPAPUS-OFFSET': '0',
      'BPAPUS-FETCH': '0'
...
export const getSalesTargetApi = data => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      executiveErpV3Api(RequestBody).then(v => {
        const {
          ReasonString,
          ResponseCode,
          ResponseData
        ...
        if (ResponseCode == 200) {
          ...
          if (response) {
            ...
            let dataObj1 = {
              ...
              'BPAPUS-PARAM': '',
              'BPAPUS-FILTER': " and CONVERT(DATETIME, DI_DATE)  BETWEEN  CONVERT(DATETIME,'" + data.FROM.substring(0, 8) + "') and (CONVERT(DATETIME,'" + data.TO.substring(0, 8) + "' ))" + " and  DT_KEY = '" + config.VANCONFIG.VANCNF_RTN_DT + "'",
...
export const getReportDataApi = (uri, data) => {
  return new Promise((resolve, reject) => {
    Request.instance.post(`/Report/${uri}`, data, {
...
export const getReportDataNoGroupApi = (uri, data) => {
  return new Promise((resolve, reject) => {
    Request.instance.post(`/Report/${uri}/nogroup`, data, {
      timeout: REPORT_REQUEST_TIMEOUT_MS
    }).then(v => {
      resolve(v.data);
    }).catch(err => {
