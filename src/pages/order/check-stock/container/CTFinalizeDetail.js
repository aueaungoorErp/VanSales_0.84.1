...
import { calculateOrderNetPriceAfterDiscount, calculateOrderProductProcessSummary, calculateOrderProductSummary, clearDisBill, createOrderSaleV3, orderAttachImage, orderReservV3, orderReturn, orderTransferV3, processOrderSale, setDisBill1, setDisBill2, setDisBillProcess, setDisCountType1, setDisCountType2, setHeaderProcessedShipDate, setOrderItems, setVDIRemark, updateOrderSale } from '../../../../action/order';
...
class CTFinalizeDetail extends Component {
          ...
          if (this.props.order.header.AR_ORDER_TYPE === 'ขายสินค้า') {
            if (this.state.paymentType !== null) {
              if (this.state.paymentType === '0') {
                ...
                if (this.state.userToken.VANCONFIG.VANCNF_ENABLE_CASH == 2 || this.state.userToken.VANCONFIG.VANCNF_CHEQUE == 2 //||
  ...
  _orderReturn = async () => {
    try {
      ...
      // );
      if (this.state.userToken.VANCONFIG.VANCNF_ENABLE_AR != 2 && this.state.returnType == '0') {
  ...
  _customerUpdateStockBalance = async () => {
    try {
      ...
      const listItems = productListItems.filter((item, index) => {
        ...
        item.DIFFERENCE = 1;
        if (item.CAS_QTY !== undefined && item.CAS_QTY !== null && item.CAS_QTY !== '' && parseInt(item.CAS_QTY) <= item.VTRD_QTY && item.DIFFERENCE > 0) {
  ...
  _setPaymentType = value => {
    if (this.state.userToken.VANCONFIG.VANCNF_ENABLE_CASHSALES == 'Y' && value == 1) {
      this._setErrorMessage(null);
      this.setState(oldState => {
        return {
          paymentType: value
    ...
    } else if (this.state.userToken.VANCONFIG.VANCNF_ENABLE_INV == 'Y' && value == 0) {
      this._setErrorMessage(null);
      this.setState(oldState => {
        return {
          paymentType: value
    ...
    } else if (this.state.userToken.VANCONFIG.VANCNF_ENABLE_CASHSALES == 'N' && value == 1) {
      this._setSubmitDisabled(true);
      this._setSuccessMessage(null);
      this._setErrorMessage('กรุณาตรวจสอบการตั้งค่าขายสด');
    } else if (this.state.userToken.VANCONFIG.VANCNF_ENABLE_INV == 'N' && value == 0) {
      this._setSubmitDisabled(true);
      this._setSuccessMessage(null);
  ...
  _setExpiryDate = async value => {
    await this.setState(oldState => {
      const formattedExpiryDate = moment(value, 'DD/MM/YYYY').format('YYYYMMDD');
  ...
  _renderItem = (item, key) => {
    ...
    return <>
      ...
      }, item.methodName === 'confirm' ? this.state.submitDisabled : this.state.disabledButton ? {
  ...
  render() {
    ...
    this.props.order.header.AR_ORDER_TYPE === 'ขายสินค้า' && this.state.userToken && this.state.userToken.VANCONFIG && this.state.userToken.VANCONFIG.VANCNF_NOV_SKU_BAL && this.state.userToken.VANCONFIG.VANCNF_NOV_SKU_BAL == 1 && this.props.order.orderProductSummary.ORDER_PROCESS_FAIL != '' && this.state.errorMessage === undefined && this.state.errorMessage != this.props.order.orderProductSummary.ORDER_PROCESS_FAIL ? this._setErrorMessage(this.props.order.orderProductSummary.ORDER_PROCESS_FAIL) : undefined;
    ...
    return <FinalizeDetail arOrderType={this.props.order.header.AR_ORDER_TYPE} vdiRemark={this.props.order.header.VDI_REMARK} orderProductSummary={this.props.order.orderProductSummary} orderProductSummaryProcessed={this.props.order} processResult={this.state.processResult} setDisBill1={this._setDisBill1} setDisBill2={this._setDisBill2} setDisType1={this._setDisType1} setDisType2={this._setDisType2} setShipDate={this._setShipDate} setExpiryDate={this._setExpiryDate} setVDIRemark={this._setVDIRemark} //หมายเหตุ
    buttonListItems={productFinalizeFormButtonGroup} renderItem={this._renderItem} successMessage={this.state.successMessage} errorMessage={this.state.errorMessage} isLoading={this.state.isLoading} paymentItems={paymentLOVItems} paymentType={this.state.paymentType} setPaymentType={this._setPaymentType} shipDate={this.state.shipDate} expiryDate={this.state.expiryDate} setReturnType={this._setReturnType} returnType={this.state.returnType} returnItems={returnLOVItems} saleDisable={this.state.saleDisable} returnDisable={this.state.returnDisable} editableDisBill={editableDisBill} changePaymentType={this._changePaymentType} />;
  }
}
