...
import { addProduct, addStockImageItem, calculateOrderProductSummary, createQuotation, getProductListItemsFromLastBillByArCode, orderAttachMultipleImages, orderUpdateQuotation, processOrderSale, removeAllProductItems, removeAllStockImageItems, setHeader, setOrderItems, setVDIRemark } from '../../../../action/order';
...
import { checkStockButtonGroup, checkStockImageButtonGroup, checkStockSummaryButtonGroup, checkStockTopButtonGroup, MainTheme } from '../../../../constant/lov';
import { printReceipt2 } from '../../../../constant/printing-pdf-lov';
import { BluetoothFinder, BplusPrinting } from '../../../../module';
import Navigator from '../../../../services/Navigator';
import { convertProductItemToOrderItem, genenrateMultipleAttachImageToServer, genenrateOrderForCreateToServer, genenrateOrderForProcessToServer, genenrateOrderForUpdateToServer, generateHeader } from '../../../../utils/Order';
import { getLoginGuID, getSettingConfig, getUserToken } from '../../../../utils/Token';
import { loadDefaultBookingExpiryDates, shouldLoadDefaultBookingExpiryDates } from '../../../../utils/docSwitchDates';
...
class CTButtonGroup extends Component {
  ...
  _check_inVANCNFDatetime = async () => {
    ...
    if (parseInt(timeArray[0] + timeArray[1]) >= parseInt(VANCONFIG.VANCONFIG.VANCNF_TIME_FM) && parseInt(timeArray[0] + timeArray[1]) <= parseInt(VANCONFIG.VANCONFIG.VANCNF_TIME_TO)) {} else {
      const errorStr = 'ไม่สามารถทำรายการได้เนื่องจากเกินช่วงเวลาที่กำหนด \n\r(' + VANCONFIG.VANCONFIG.VANCNF_TIME_FM.substring(0, 2) + ':' + VANCONFIG.VANCONFIG.VANCNF_TIME_FM.substring(2, 4) + ' น. - ' + VANCONFIG.VANCONFIG.VANCNF_TIME_TO.substring(0, 2) + ':' + VANCONFIG.VANCONFIG.VANCNF_TIME_TO.substring(2, 4) + ' น.)';
      this.setState({
        _errormsg: errorStr
      });
      return;
  ...
  _setButtonGroup = () => {
    ...
    if (routeName === 'OrderCheckStock') this.props.position === 'top' ? this._listItems = checkStockTopButtonGroup : this._listItems = checkStockButtonGroup;else if (routeName === 'OrderCheckStockSummary') this._listItems = checkStockSummaryButtonGroup;else if (routeName === 'OrderCheckStockImageItems') this._listItems = checkStockImageButtonGroup;
  };
  ...
  _renderItem = (item, key) => {
    ...
    const disabled = this.props.order.productListItems.length <= 0 && item.methodName === 'confirm' ? true : false;
    return <TouchableOpacity key={key} style={[item.methodName === 'process' && this.state.submitDisabled == true ? greentStyle : item.buttonStyle, item.containerStyle, {
      ...
      this._onPress(item);
        ...
        titleStyle: {
          ...
          {item.title === 'พิมพ์' && this.props.bluetooth.printingType !== 'BLUETOOTH' ? item.subTitle : item.title}
        </Text>
      </TouchableOpacity>;
  };
  _onPress = async item => {
    ...
    const printType = routes[index].params !== undefined && routes[index].params !== null && routes[index].params.printType !== undefined && routes[index].params.printType !== null ? routes[index].params.printType : null;
    const processResult = routes[index].params !== undefined && routes[index].params !== null && routes[index].params.processResult !== undefined && routes[index].params.processResult !== null ? routes[index].params.processResult : null;
    const orderProductSummary = routes[index].params !== undefined && routes[index].params !== null && routes[index].params.orderProductSummary !== undefined && routes[index].params.orderProductSummary !== null ? routes[index].params.orderProductSummary : null;
  ...
  _getLastBillDialog = () => Alert.alert('ประกาศ', 'ต้องการทำรายการเหมือนบิลล่าสุดหรือไม่?', [{
    text: 'ยกเลิก',
    onPress: () => {},
    style: 'cancel'
  }, {
  ...
  _removeAlertDialog = () => Alert.alert('ประกาศ', 'คุณต้องการลบข้อมูลสินค้าทั้งหมด?', [{
    text: 'ยกเลิก',
    onPress: () => {},
    style: 'cancel'
  }, {
    text: 'ยืนยัน',
    onPress: () => this._removeAll()
  }], {
    cancelable: false
  });
  _removeAll = () => {
    ...
    try {
      ...
      if (this.props.bluetooth.state == 'connected') {
        ...
        if (response.ResponseCode === '200') {
          ...
          const summary = this.props.order.orderProductSummaryProcessed ? false : this.props.order.orderProductSummaryProcessed;
          ...
          BplusPrinting.printReceipt(type !== 'quotation' ? this.props.order.header : {
            ...this.props.order,
            orderProductSummary: orderProductSummary
          }, this.props.order.productListItemsPRTProcessed, processResult, userToken.VANCONFIG, userToken.COMPANYINFO, type !== 'transfer' ? this.props.customer.item.INFO : this.props.order.header, printTime, type, 0, isDiscountBath, this.props.order && Array.isArray(this.props.order.paymentMethod) && this.props.order.paymentMethod.length > 0 ? this.props.order?.paymentMethod[0] : {});
  ...
  _alertErrorDialog = message => Alert.alert('ประกาศ', message.toString(), [{
    text: 'ตกลง',
    onPress: () => {},
    style: 'cancel'
  }], {
  ...
  _createQuatation = async () => {
    try {
      ...
      if (parseInt(timeArray[0] + timeArray[1]) >= parseInt(VANCONFIG.VANCNF_TIME_FM) && parseInt(timeArray[0] + timeArray[1]) <= parseInt(VANCONFIG.VANCNF_TIME_TO)) {
        const config = await getSettingConfig();
        let response = await this.props.processOrderSale(genenrateOrderForProcessToServer(this.props.order, true, 1, typeof this.props.order.orderProductSummary.DIS_COUNT_TYPE1 === 'undefined' ? true : this.props.order.orderProductSummary.DIS_COUNT_TYPE1, this.props.order.header.AR_ORDER_TYPE), this.props.order, config.VANCONFIG);
        const {
          ResponseData,
          ResponseCode,
          ReasonString
        ...
        if (ResponseCode == '200') {
          ...
          if (this.props.order.header.VDI_USER_REF === null) {
            response = await this.props.createQuotation(genenrateOrderForCreateToServer(this.props.order, this.props.mile.item.mileage, this.props.geolocation.position), v3GUID, userToken.VANCONFIG);
          } else {
            response = await this.props.orderUpdateQuotation(genenrateOrderForUpdateToServer(genenrateOrderForCreateToServer(this.props.order, this.props.mile.item.mileage, this.props.geolocation.position), this.props.order.header));
          }
        ...
        const errorStr = 'ไม่สามารถทำรายการได้เนื่องจากเกินช่วงเวลาที่กำหนด ( ' + config.VANCONFIG.VANCNF_TIME_FM[0] + config.VANCONFIG.VANCNF_TIME_FM[1] + ':' + config.VANCONFIG.VANCNF_TIME_FM[2] + config.VANCONFIG.VANCNF_TIME_FM[3] + ' - ' + config.VANCONFIG.VANCNF_TIME_TO[0] + config.VANCONFIG.VANCNF_TIME_TO[1] + ':' + config.VANCONFIG.VANCNF_TIME_TO[2] + config.VANCONFIG.VANCNF_TIME_TO[3] + ' )';
  ...
  _orderAttachMultipleImages = async () => {
    try {
      response = await this.props.orderAttachMultipleImages(genenrateMultipleAttachImageToServer(this.props.order.headerProcessed.VDI_KEY, this.props.order.headerProcessed.VDI_AR, this.props.order.stock.imageItems));
      const {
        RESULT_DATA,
        STATUS,
        ERROR_MESSAGES
    ...
    try {
      ...
      if (responseData.RECORD_COUNT != '0' && ResponseCode == 200) {
        ...
        let html = printReceipt2({
          ...this.props.order,
          orderProductSummary: orderProductSummary
        }, this.props.order.productListItemsPRTProcessed, processResult, userToken.VANCONFIG, userToken.COMPANYINFO, type !== 'transfer' ? this.props.customer.item.INFO : this.props.order.header, printTime, type, 0, isDiscountBath);
  ...
  _pdfAlertDialog = path => Alert.alert('ประกาศ', path, [{
    text: 'แสดง',
  ...
  render() {
    ...
    return <View>
        <Dialog.Container visible={this.state.remarkDialogVisible}>
          ...
          <Dialog.Input onChangeText={value => this.props.setVDIRemark(value)} style={{
          borderWidth: 0.5
        }} />
          <Dialog.Button label="ยกเลิก" onPress={() => {
          this._setState('remarkDialogVisible', false);
          this.props.setVDIRemark('');
        }} />
          <Dialog.Button label="ยืนยัน" onPress={() => this._remarkDialogConfirm()} />
        </Dialog.Container>
        <>
          {!this.state._errormsg ? <ButtonGroup listItems={this.state._errormsg ? null : this._listItems} renderItem={this._renderItem} successMessage={this.state.successMessage} errorMessage={this.state.errorMessage} isLoading={this.state.isLoading} pattern={this.props.screen === 'Index' ? 'A' : 'B'} /> : this.props.position === 'top' ? <Text style={styles.red}>
              {'\n\n'}
              {this.state._errormsg}
            </Text> : <ButtonGroup listItems={this._listItems} renderItem={this._renderItem} successMessage={this.state.successMessage} errorMessage={this.state.errorMessage} isLoading={this.state.isLoading} pattern={this.props.screen === 'Index' ? 'A' : 'B'} />}
        </>
      </View>;
  }
}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    processOrderSale: (data, order, vanConfig) => dispatch(processOrderSale(data, order, vanConfig)),
    createQuotation: (data, V3GUID, vanConfig) => dispatch(createQuotation(data, V3GUID, vanConfig)),
    ...
    orderAttachMultipleImages: data => dispatch(orderAttachMultipleImages(data)),
    removeAllProductItems: () => dispatch(removeAllProductItems()),
    setProductInitialState: () => {
      dispatch(setProductInitialState());
    },
    getProductListItemsFromLastBillByArCode: (v3GUID, vancnf_machine) => dispatch(getProductListItemsFromLastBillByArCode(v3GUID, vancnf_machine)),
