...
import { addProduct, addProducts, calculateOrderProductSummary, getProductListItemsFromLastBillByArCode, removeAllProductItems, removeAllSwipeList, setSwipeCurrent } from '../../../action/order';
import { setInitialState } from '../../../action/product';
import { serverReady } from '../../../api/setting';
import { MainTheme, orderSalesButtonGroup, orderSalesButtonGroupAddSCR, orderSalesButtonGroupEditSCR, orderSalesFooterButtonGroup, orderSalesReturnButtonGroup } from '../../../constant/lov';
import Navigator from '../../../services/Navigator';
import { convertProductItemToOrderItem, convertProductItemToOrderItemSCR } from '../../../utils/Order';
import { LAST_BILL_QUOTATION_WARNING_MESSAGE, getLatestSoldQuotationGoodsCodes } from '../../../utils/LastBillValidation';
...
class CTButtonGroup extends Component {
  ...
  _renderItem = (item, key) => {
    const disabled = this.props.order.productListItems.length <= 0 && item.title === 'ตกลง' ? true : false;
  ...
  _onPress = async item => {
    ...
    if (item.methodType === 'new-page') {
      if (item.screen === 'ProductAddTo') {
        ...
        if (userToken.VANCONFIG.VANCNF_KEYIN_SCR == 1 || this.props.order.header.AR_ORDER_TYPE === 'รับคืนสินค้า' || this.props.order.header.AR_ORDER_TYPE === 'โอนย้ายสินค้า') {
          ...
          Navigator.navigate(item.screen, {
            ...
            confirmMethod: async items => {
              await this.props.addProducts(convertProductItemToOrderItemSCR(items));
              await this.props.calculateOrderProductSummary();
              Navigator.back();
            }
          });
        ...
        if (item.screen === 'OrderSalesFinalize' && shouldLoadDefaultBookingExpiryDates(arOrderType)) {
  ...
  _alertDialog = () => Alert.alert('ประกาศ', 'คุณแน่ใจว่าจะออกจากหน้าจอนี้', [{
    text: 'ยกเลิก',
    onPress: () => {},
    style: 'cancel'
  }, {
  ...
  _getProductListItemsFromLastBillByArCode = async (goodsCodes = null) => {
    try {
      ...
      const userToken = await getUserToken();
      await this.props.getProductListItemsFromLastBillByArCode(v3GUID, userToken.VANCONFIG.VANCNF_MACHINE, goodsCodes);
      await this.props.calculateOrderProductSummary();
    } catch (error) {
      console.log(error);
    }
  ...
  _handleLastBillPress = async () => {
    try {
      ...
      if (!validationResult.allowed) {
        this._showLatestItemModal(validationResult.message || LAST_BILL_QUOTATION_WARNING_MESSAGE);
        return;
      }
      this.props.order.errorMessage = null;
      this._removeAll();
      this._getProductListItemsFromLastBillByArCode(validationResult.goodsCodes);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  render() {
    if (this.props.hasFooterButton) {
      return <ButtonGroup listItems={orderSalesFooterButtonGroup} renderItem={this._renderItem} />;
    } else {
      let buttonGroup = null;
      if (this.props.order.header.AR_ORDER_TYPE === 'รับคืนสินค้า' || this.props.order.header.AR_ORDER_TYPE === 'โอนย้ายสินค้า') {
      ...
      return <>
          {!this.state._errormsg ? <ButtonGroup listItems={this.state._errormsg ? null : buttonGroup} renderItem={this._renderItem} /> : <Text style={styles.red}>
              {'\n\n'}
              {this.state._errormsg}
            </Text>}
          <Modal transparent={true} visible={this.state.isLatestItemModalVisible} animationType="fade" onRequestClose={this._closeLatestItemModal}>
            <View style={styles.modalOverlay}>
              <View style={styles.modalCard}>
                ...
                <TouchableOpacity style={styles.modalButton} onPress={this._closeLatestItemModal} activeOpacity={0.7}>
