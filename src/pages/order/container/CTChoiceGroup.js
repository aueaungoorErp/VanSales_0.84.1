...
class CTChoiceGroup extends Component {
  ...
  _renderItem = ({
    ...
    return <View style={styles.card}>
        <TouchableOpacity onPress={() => this._onPress(item)} style={styles.touchable}>
          <Image style={styles.image} resizeMode="contain" source={item.imgSrc} />
  ...
  _buildTimeDialogMessage = vanConfig => {
    ...
    return 'ไม่สามารถทำรายการได้เนื่องจากเกินช่วงเวลาที่กำหนด\n\r(' + timeFrom.substring(0, 2) + ':' + timeFrom.substring(2, 4) + ' น. - ' + timeTo.substring(0, 2) + ':' + timeTo.substring(2, 4) + ' น.)';
  };
  ...
  _onPress = async item => {
    ...
    if (item.screen == 'OrderSurvey') {
      const supported = await Linking.canOpenURL(userToken.VANCONFIG.VANCNF_URL_SURVEY);
      if (supported) {
        await Linking.openURL(userToken.VANCONFIG.VANCNF_URL_SURVEY);
      } else {
        Alert.alert(`Don't know how to open this URL: ${userToken.VANCONFIG.VANCNF_URL_SURVEY}`);
      }
    } else if (item.screen == 'OrderVisit') {
      // const supported = await Linking.canOpenURL('https://google.com');
      const supported = await Linking.canOpenURL(userToken.VANCONFIG.VANCNF_URL_VISIT);
      if (supported) {
        await Linking.openURL(userToken.VANCONFIG.VANCNF_URL_VISIT);
      } else {
        Alert.alert(`Don't know how to open this URL: ${userToken.VANCONFIG.VANCNF_URL_VISIT}`);
      }
    } else {
    ...
    if (userToken) {
      orderChoiceButtonGroup.map((value, index) => {
        if (value.orderType === 'return-product') {
          ...
          if (userToken.VANCONFIG.VANCNF_ENABLE_INV == 'Y' || userToken.VANCONFIG.VANCNF_ENABLE_CASH == 'Y') {
            result.push(value);
          }
        ...
        <Modal transparent animationType="fade" visible={this.state.isTimeDialogVisible} onRequestClose={this._closeTimeDialog}>
          <View style={styles.modalOverlay}>
            <View style={styles.dialogCard}>
              ...
              <TouchableOpacity style={styles.dialogButton} onPress={this._closeTimeDialog} activeOpacity={0.8}>
