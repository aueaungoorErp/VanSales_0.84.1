...
import { calculateOrderProductSummary, editProduct, getProductListItemsFromLastBillByArCode, pushSwipeList, removeAllSwipeList, removeProductItem, setSwipeCurrent } from '../../../action/order';
...
class CTListItems extends Component {
  ...
  _autoPullOrderLastBill = async () => {
    ...
    const userToken = await getUserToken();
    if (!disabledAutoLoad === true && userToken.VANCONFIG.VANCNF_AS_PREVIOUS == 2 && userToken.VANCONFIG.VANCNF_KEYIN_SCR == 1 && (this.props.order.header.AR_ORDER_TYPE === 'ขายสินค้า' || this.props.order.header.AR_ORDER_TYPE === 'จองสินค้า') && this.props.order.header.VDI_USER_REF === null) {
      ...
      if (!validationResult.allowed) {
        this.setState({
          ...
          latestItemModalMessage: validationResult.message || LAST_BILL_QUOTATION_WARNING_MESSAGE
        });
        return;
      }
  ...
  _renderItem5INCH = ({
    ...
    return <ListItem containerStyle={mainDivider} bottomDivider>
            ...
            flexDirection: 'row'
              ...
              paddingHorizontal: 5
                ...
                justifyContent: 'space-around'
                  ...
                  <Icon name="pencil" color={MainTheme.colorPrimary} size={36} type={'font-awesome'} onPress={() => {
                  this._editProductItem(item, index);
                }} />

                  <Icon name="trash-o" color={MainTheme.colorPrimary} size={36} type={'font-awesome'} onPress={() => {
                ...
                justifyContent: 'space-between'
                  ...
                  fontSize: hp('1.7%')
                    ...
                    //     .replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                      ...
                      {item.VTRD_U_DSC ? parseFloat(item.VTRD_U_DSC).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',') : parseFloat(item.GOODS_TOTAL_DISCOUNT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                    </Text>
                  </View>
                  <View style={{
                flexDirection: 'row',
  ...
  render() {
    return <>
        <ListItems listItems={this.props.order.productListItems} renderItem={Dimensions.get('window').width > MOBILE5INCH ? this._renderItem : this._renderItem5INCH} refreshing={this.props.order.isLoading} errorMessage={this.props.order.errorMessage} onRefresh={this._onRefresh} />
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    getProductListItemsFromLastBillByArCode: (V3GUID, vancnf_machine, goodsCodes) => dispatch(getProductListItemsFromLastBillByArCode(V3GUID, vancnf_machine, goodsCodes))
  };
};
