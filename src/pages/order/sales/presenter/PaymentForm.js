...
const PaymentForm = props => {
  ...
  useEffect(() => {}, [bankFileListItems, bankFileItem]);
  ...
  const getSelectedQrContent = promptPay => {
    ...
    return safeQrContentListItems.find(item => String(item.QRCT_KEY) === String(promptPay) || String(item.QRCT_CODE) === String(promptPay)) || null;
  };
  const defaultQrContent = safeQrContentListItems[0] || null;
  const isQrTemplatePayload = content => {
    const normalizedContent = String(content || '').trim();
    return normalizedContent.startsWith('000201') || normalizedContent.includes('5303764') && normalizedContent.includes('5802TH');
  };
  ...
  const formattedTotalPrice = safeTotalPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
  ...
  const setTextcashin = item => {
    setcashin(item);
    setAmtcashin(item.replace(/,/g, '').replace(/-$/, '').replace(/\.$/, ''));
  };
  const setTexttranFerin = item => {
    settranFerin(item);
    setAmttranferin(item.replace(/,/g, '').replace(/-$/, '').replace(/\.$/, ''));
  };
  const setTextqRCodein = item => {
    setqRCodein(item);
    setAmtQrin(item.replace(/,/g, '').replace(/-$/, '').replace(/\.$/, ''));
  };
  const setTextchequein = item => {
    setchequein(item);
    setAmtchequein(item.replace(/,/g, '').replace(/-$/, '').replace(/\.$/, ''));
  };
  const setTextotherin = item => {
    setotherin(item);
    setAmtOtherin(item.replace(/,/g, '').replace(/-$/, '').replace(/\.$/, ''));
  };
  ...
  const checkpayment = (amount, promptPay) => {
    ...
    const fallbackQrSeed = [rawQrContent, qrContent?.BNKAC_CODE, qrContent?.QRCT_CODE, qrContent?.QRCT_NAME, qrContent?.BNKAC_NAME, !qrContent && isUsableQrSeed(rawPromptPay) ? rawPromptPay : null].map(item => String(item || '').trim()).find(item => isUsableQrSeed(item));
  ...
  const genQrPayment = (amount, promptPay) => {
    ...
    let pp_str = '000201010211' + '29' + field_29.length + field_29 + '5303764' + pp_amount + '5802TH' + '6304';
  ...
  const qrPaymentValue = isDirectQrPayload(qrCode) ? String(qrCode).trim() : checkpayment(qrAmount, qrContentItem) || checkpayment(qrAmount, qrCode);
  ...
  return <Container>
      <Content>
        ...
        {showCashCard ? <View style={styles.paymentCard}>
          ...
          backgroundColor: checkedItems.item1 ? '#f0ffff' : 'white'
            ...
            borderBottomWidth: 0
                ...
                <CheckBox title="เงินสด"
            //checked={paymentType === 'cash'}
            checked={checkedItems.item1} checkedColor={MainTheme.colorTertiary} containerStyle={[styles.checkBoxStyle, {
              backgroundColor: checkedItems.item1 ? '#f0ffff' : 'white'
            }]} textStyle={{
              fontSize: hp('1.6%')
            }} onPress={async () => {
            ...
            backgroundColor: checkedItems.item1 ? '#f0ffff' : 'white'
                ...
                <Input editable={checkedItems.item1} placeholder="ยอดเงิน"
            //placeholderTextColor={MainTheme.placeholerTextInput}
            placeholderTextColor={checkedItems.item1 ? '#808080' : MainTheme.placeholerTextInput} value={checkedItems.item1 === true ? cashin : ''} // Number(cashin).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',') : ""} //.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',') : null}
            maxLength={14} style={{
              fontSize: 14,
              paddingVertical: 0,
              height: 30
            }} keyboardType="numeric" onBlur={() => handleBlur('cash')} onChangeText={value => {
              setTextcashin(value);
              setremainConfirm(false);
              setremainoptionItem(null);
            }} />
        ...
        {showTransferQrCard ? <View style={styles.paymentCard}>
            <View style={styles.paymentCardHeader}>
              <Text style={styles.paymentCardTitle} allowFontScaling={false}>
                โอนและ QR Code
              </Text>
          ...
          backgroundColor: checkedItems.item2 ? '#f0ffff' : 'white'
            ...
            checked={checkedItems.item2} checkedColor={MainTheme.colorTertiary} containerStyle={[styles.checkBoxStyle, {
              backgroundColor: checkedItems.item2 ? '#f0ffff' : 'white'
            }]} textStyle={{
              fontSize: hp('1.6%')
            }} onPress={async () => {
            ...
            backgroundColor: checkedItems.item2 ? '#f0ffff' : 'white'
                ...
                <Input editable={checkedItems.item2} placeholder="ยอดเงิน" placeholderTextColor={checkedItems.item2 ? '#808080' : MainTheme.placeholerTextInput}
            //value={checkedItems.item2 == true ? (Number(totalPrice) - Number(tranFerin) - Number(cashin)).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',') : null}
            value={checkedItems.item2 === true ? tranFerin : ''} maxLength={14} style={{
              fontSize: 14,
              paddingVertical: 0,
              height: 30
            }} keyboardType="numeric" onBlur={() => handleBlur('transfer')} onChangeText={value => {
            ...
            {showQrCodeOption && safeUserToken.VANCONFIG.VANCNF_BANK_QRCODE_USE !== 2 ? <Item style={[styles.checkBoxSection, {
          backgroundColor: checkedItems.item3 ? '#f0ffff' : 'white'
            ...
            borderBottomWidth: 0
                  ...
                  <CheckBox title="QRCode"
            //checked={paymentType === 'qrcode'}
            checked={checkedItems.item3} checkedColor={MainTheme.colorTertiary} containerStyle={[styles.checkBoxStyle, {
              backgroundColor: checkedItems.item3 ? '#f0ffff' : 'white'
            }]} textStyle={{
              fontSize: hp('1.6%')
            }} onPress={async () => {
            ...
            backgroundColor: checkedItems.item3 ? '#f0ffff' : 'white'
                  ...
                  <Input editable={checkedItems.item3} placeholder="ยอดเงิน" placeholderTextColor={checkedItems.item3 ? '#808080' : MainTheme.placeholerTextInput}
            ...
            maxLength={14} style={{
              ...
              paddingVertical: 0,
              height: 30
            }} keyboardType="numeric" onBlur={() => handleBlur('qrcode')} onChangeText={value => {
            ...
            {checkedItems.item3 && showQrCodeOption && showBBLQrPayment ? <View style={styles.bblQrInfoBox}>
                <Text style={styles.bblQrInfoText} allowFontScaling={false}>
                  ระบบจะสร้าง QR Code จาก BBL QR Payment อัตโนมัติ
                </Text>
                {isQRCodeGenerating ? <View style={styles.bblQrLoadingRow}>
                    <ActivityIndicator size="small" color={MainTheme.colorPrimary} />
                    <Text style={styles.bblQrLoadingText} allowFontScaling={false}>
                      กำลังสร้าง QR Code...
                    </Text>
                  </View> : null}
              </View> : <Item style={[styles.otherPickerSection, {
          backgroundColor: checkedItems.item2 ? '#f0ffff' : checkedItems.item3 ? '#f0ffff' : 'transparent'
        }]}>
                <Form style={[styles.inputSection2, {
            borderBottomColor: 'transparent',
            borderBottomWidth: 0,
            height: 40,
            padding: 0
          }]}>
                  <RNPickerSelect items={checkedItems.item2 == true ? bankAccount : checkedItems.item3 == true && showQrCodeOption ? qrCodeContent : []} disabled={checkedItems.item2 == true ? !checkedItems.item2 : checkedItems.item3 == true && showQrCodeOption ? !checkedItems.item3 : true} onValueChange={value => {
              checkedItems.item2 == true ? setBankAccountItem ? setBankAccountItem(value) : null : checkedItems.item3 == true && showQrCodeOption ? setqrContentItem ? setqrContentItem(value) : null : null;
            }} value={checkedItems.item2 == true ? bankAccountItem : checkedItems.item3 == true && showQrCodeOption ? qrContentItem : null} style={getBorderlessPickerStyle(checkedItems.item2 || checkedItems.item3 && showQrCodeOption)} placeholder={{
              label: 'เลือก',
              value: null
            }} placeholderTextColor={checkedItems.item2 ? '#808080' : MainTheme.placeholerTextInput} textInputProps={{
              underlineColorAndroid: 'transparent',
              underlineColor: 'transparent'
            }} useNativeAndroidPickerStyle={false} Icon={() => {
              return <AntDesign name="down" size={20} color={MainTheme.colorPrimary} style={{
                marginTop: 0
              }} />;
            }} />
                </Form>
        ...
        {showChequeCard ? <View style={styles.paymentCard}>
            <View style={styles.paymentCardHeader}>
              <Text style={styles.paymentCardTitle} allowFontScaling={false}>
                เช็ค
              </Text>
            ...
            backgroundColor: checkedItems.item4 ? '#f0ffff' : 'white'
              ...
              checked={checkedItems.item4} checkedColor={MainTheme.colorTertiary} containerStyle={[styles.checkBoxStyle, {
                backgroundColor: checkedItems.item4 ? '#f0ffff' : 'white'
              }]} textStyle={{
                fontSize: hp('1.6%')
              }} onPress={async () => {
              ...
              backgroundColor: checkedItems.item4 ? '#f0ffff' : 'white'
                    ...
                    <Input editable={checkedItems.item4} placeholder="ยอดเงิน" placeholderTextColor={checkedItems.item4 ? '#808080' : MainTheme.placeholerTextInput}
              ...
              maxLength={14} style={{
                ...
                paddingVertical: 0,
                height: 30
              }} keyboardType="numeric" onBlur={() => handleBlur('cheque')} onChangeText={value => {
            ...
            backgroundColor: checkedItems.item4 ? '#f0ffff' : 'transparent'
              ...
              padding: 0
                    ...
                    <RNPickerSelect key={chequeBankPickerKey} items={bankFiles} disabled={!checkedItems.item4 || !isChequeBankListReady} onValueChange={value => {
                setBankFileItem ? setBankFileItem(value) : null;
              }} value={checkedItems.item4 == true ? bankFileItem : null} style={getBorderlessPickerStyle(checkedItems.item4)} placeholder={{
                label: isChequeBankListReady ? 'เลือก' : 'กำลังโหลดข้อมูลธนาคาร...',
                value: null
              }} placeholderTextColor={checkedItems.item4 ? '#808080' : MainTheme.placeholerTextInput} textInputProps={{
                underlineColorAndroid: 'transparent',
                underlineColor: 'transparent'
              }} useNativeAndroidPickerStyle={false} Icon={() => {
                return <AntDesign name="down" size={20} color={MainTheme.colorPrimary} style={{
                ...
                {checkedItems.item4 && !isChequeBankListReady ? <Text allowFontScaling={false} style={styles.chequeLoadingText}>
            ...
            backgroundColor: checkedItems.item4 ? '#f0ffff' : 'white'
              ...
              placeholder="เลขที่เช็ค" placeholderTextColor={checkedItems.item4 ? '#808080' : MainTheme.placeholerTextInput} value={chequeNo} maxLength={8} style={{
                fontSize: 14,
                paddingVertical: 0,
                height: 35
            ...
            backgroundColor: checkedItems.item4 ? '#f0ffff' : 'white'
              ...
              height: 35
                ...
                flex: 1
                      ...
                      <IDatePicker disabled={!checkedItems.item4} value={chequeDate} onDateChange={setChequeDate} style={{
                  fontSize: 14,
                  paddingVertical: 0
                }} />
                    </View>
        ...
        {showOtherCard ? <View style={styles.paymentCard}>
            <View style={styles.paymentCardHeader}>
              <Text style={styles.paymentCardTitle} allowFontScaling={false}>
                ช่องทางอื่นๆ
              </Text>
            ...
            backgroundColor: checkedItems.item5 ? '#f0ffff' : 'white'
              ...
              borderBottomWidth: 0
                    ...
                    <CheckBox title="อื่นๆ" checked={checkedItems.item5} checkedColor={MainTheme.colorTertiary} containerStyle={[styles.checkBoxStyle, {
                backgroundColor: checkedItems.item5 ? '#f0ffff' : 'white'
              }]} textStyle={{
                fontSize: hp('1.6%')
              }} onPress={async () => {
              ...
              backgroundColor: checkedItems.item5 ? '#f0ffff' : 'white'
                    ...
                    <Input editable={checkedItems.item5} placeholder="ยอดเงิน" placeholderTextColor={checkedItems.item5 ? '#808080' : MainTheme.placeholerTextInput} value={checkedItems.item5 === true ? otherin : ''} // Number(cashin).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',') : ""} //.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',') : null}
              maxLength={14} style={{
                fontSize: 14,
                paddingVertical: 0,
                height: 30
              }} keyboardType="numeric" onBlur={() => handleBlur('other')} onChangeText={value => {
            ...
            backgroundColor: checkedItems.item5 ? '#f0ffff' : 'transparent'
              ...
              padding: 0
                    ...
                    <RNPickerSelect items={otherPayment} disabled={!checkedItems.item5} onValueChange={value => {
                setotherPaymentItem ? setotherPaymentItem(value) : null;
              }} style={getBorderlessPickerStyle(checkedItems.item5)} value={checkedItems.item5 == true ? otherPaymentItem : null} placeholder={{
                label: 'เลือก',
                value: null
              }} textInputProps={{
                underlineColorAndroid: 'transparent',
        ...
        alignItems: 'center'
          ...
          <IButtonGroupCustom listItems={buttonListItems} renderItem={renderItem} style={{
          flexDirection: 'row',
          justifyContent: 'space-between'
        }} />
        </View>

        <Modal transparent={true} visible={isDialogOpen} onRequestClose={() => {
        setState ? setState('isDialogOpen', false) : null;
      }}>
            ...
            <View style={styles.remainModalCard}>
              <View style={styles.remainModalHeader}>
                <Text style={styles.remainModalHeaderText} allowFontScaling={false}>
                  ระบุหมายเหตุการชำระ{differValue < 0 ? 'ขาด' : 'เกิน'}{' '}
                  {Math.abs(differValue).toFixed(2)} บาท
                </Text>
              </View>
                ...
                {<>
                    <Item style={styles.remainModalPickerWrapper}>
                      <Form style={styles.remainModalPickerForm}>
                        <RNPickerSelect items={remainoption}
                    //disabled={!true}
                    onValueChange={value => {
                      setremainoptionItem ? setremainoptionItem(value) : null;
                    }} value={remainoptiontItem} style={{
                      iconContainer: {
                        top: -3,
                        right: 0
                      },
                      ...
                      label: 'เลือก',
                      value: null
                    }} placeholderTextColor={'#808080'} textInputProps={{
                      underlineColorAndroid: 'transparent',
                      underlineColor: 'transparent'
                    }} useNativeAndroidPickerStyle={false} Icon={() => {
                      return <AntDesign name="down" size={20} color={MainTheme.colorPrimary} style={{
              ...
              <IButtonGroupCustom listItems={buttonListItems} renderItem={renderItemRemainOption} style={remainOptionButtonGroupStyles} />
            </View>
          </View>
        </Modal>

        <Modal animationType="fade" transparent={true} visible={showQrCodeOption && isQRCodeDialogOpen && screenFocused !== false} onRequestClose={() => {
        qrDialogBusy ? null : setState ? setState('isQRCodeDialogOpen', false) : null;
      }}>
          <View style={styles.qrModalBackdrop}>
            <View style={styles.qrModalCard}>
              <View style={styles.qrModalHeader}>
                <Text style={styles.qrModalHeaderText} allowFontScaling={false}>
              ...
              <View style={styles.qrCodeArea}>
                ...
                {qrPaymentValue ? <QRCode value={qrPaymentValue} size={240} logo={qrLogo} logoSize={60} logoBackgroundColor="transparent" /> : <Text style={styles.qrEmptyText} allowFontScaling={false}>
              ...
              <View style={styles.qrModalFooter}>
                ...
                <View style={styles.qrAmountRow}>
                  <Text style={styles.qrAmountLabel} allowFontScaling={false}>
                    จำนวนเงิน
                  </Text>
                  <Text style={styles.qrAmountValue} allowFontScaling={false}>
                ...
                <View style={styles.qrActionRow}>
                  <Button large containerStyle={styles.qrActionButtonContainer} buttonStyle={styles.qrConfirmButton} titleStyle={styles.qrConfirmButtonText} disabledStyle={styles.qrConfirmButtonDisabled} disabledTitleStyle={styles.qrConfirmButtonDisabledText} icon={qrDialogBusy ? <ActivityIndicator size="small" color="#FFFFFF" style={styles.qrButtonSpinner} /> : null} title={showBBLQrPayment ? isBBLPaymentChecking ? 'กำลังตรวจสอบ...' : 'ยืนยันการชำระ' : 'ตกลง'} disabled={qrDialogBusy} onPress={() => {
                  if (showBBLQrPayment) {
                  ...
                  <Button large containerStyle={[styles.qrActionButtonContainer, styles.qrCancelButtonContainer]} buttonStyle={styles.qrCancelButton} title={'ยกเลิก'} titleStyle={styles.qrCancelButtonText} disabledStyle={styles.qrCancelButtonDisabled} disabledTitleStyle={styles.qrCancelButtonDisabledText} disabled={qrDialogBusy} onPress={() => {
                  setState ? setState('isQRCodeDialogOpen', false) : null;
                }} />
                </View>
              </View>
