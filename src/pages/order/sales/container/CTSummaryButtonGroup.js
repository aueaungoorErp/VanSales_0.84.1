...
import { addVisitImageItem, createDocVisit, removeAllVisitImageItems } from '../../../../action/order';
...
class CTSummaryButtonGroup extends Component {
    ...
    try {
      ...
      if (granted === PermissionsAndroid.RESULTS.GRANTED) {
        ...
        if (responseData.RECORD_COUNT != 0) {
          ...
          const html = printReceipt2(this.props.order, this.props.order.productListItemsPRTProcessed, processResult, userToken.VANCONFIG, userToken.COMPANYINFO, type !== 'transfer' ? this.props.customer.item.INFO : this.props.order.header, printTime, type, 0, isDiscountBath);
    ...
    return <TouchableOpacity key={key} style={[item.buttonStyle, item.containerStyle, {
      ...
      this._onPress(item);
        ...
        fontSize: hp('1.8%')
          ...
          {printType === 'transfer' && key == 0 ? this.props.bluetooth.printingType !== 'BLUETOOTH' ? item.subTitle2 : item.subTitle : item.title === 'พิมพ์ใบเสร็จ' && this.props.bluetooth.printingType !== 'BLUETOOTH' ? item.subTitle2 : item.title}
        </Text>
      </TouchableOpacity>;
  };
  _onPress = async item => {
  ...
  _pdfAlertDialog = path => Alert.alert('ประกาศ', `${RNFS.ExternalStorageDirectoryPath}/Documents/bplus_vansales/${path.split('/').pop()}`, [{
    text: 'แสดง',
  ...
  render() {
    return <SummaryButtonGroup printingType={this.props.bluetooth.printingType} listItems={orderSummaryFormButtonGroup} renderItem={this._renderItem} errorMessage={this.state.errorMessage} isLoading={this.state.isLoading} />;
  }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(CTSummaryButtonGroup);