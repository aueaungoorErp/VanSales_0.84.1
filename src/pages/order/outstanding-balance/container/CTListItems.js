...
class CTListItems extends React.Component {
  ...
  _onScroll = async event => {
    ...
    !this.props.outstandingBalance.origin.isLoading
      ? await this.props.searchList(true)
      : null;
  };
      ...
      <ListItem
        title={
          <View style={{ flexDirection: 'row' }}>
            <CheckBox
              checked={
                this.props.outstandingBalance.origin.listItems.length ===
                this.props.outstandingBalance.origin.checkNumber
              }
              checkedColor={'#0020C2'}
        ...
        title={
          <View style={{ flexDirection: 'row' }}>
            ...
            <Text
              ...
              allowFontScaling={false}
            >
              {item.ARD_A_AMT.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
            </Text>
            <Text
              style={{
                width: 100,
              ...
              allowFontScaling={false}
            >
              {item.OUTSTANDING.toFixed(2).replace(
                /\B(?=(\d{3})+(?!\d))/g,
                ',',
              )}
            </Text>
  ...
  render() {
    return (
      <ListItems
        ...
        isError={
          this.props.outstandingBalance.origin.isError &&
          this.props.outstandingBalance.origin.listItems.length == 0
        }
        isNotFound={
          this.props.outstandingBalance.origin.isNotFound &&
          this.props.outstandingBalance.origin.listItems.length == 0
        }
        isSnackBarVisible={
          this.props.outstandingBalance.origin.isError &&
