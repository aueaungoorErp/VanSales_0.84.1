  ...
  _setCreateCheckedItems = async (keyParent, keyChildren, value) => {
    ...
    if (keyChildren === 'checked') {
      ...
      if (keyParent === 'cash') {
        ...
        payItems[0] = VPH_PAY_AMT.toFixed(2).replace(
          /\B(?=(\d{3})+(?!\d))/g,
          ',',
        );
        payItems[1] = null;
        ...
        booleanItems[2] = false;
        payItems[0] = null;
        payItems[1] = VPH_PAY_AMT.toFixed(2).replace(
          /\B(?=(\d{3})+(?!\d))/g,
          ',',
        );
        payItems[2] = null;
        ...
        payItems[0] = null;
        payItems[1] = null;
        payItems[2] = VPH_PAY_AMT.toFixed(2).replace(
          /\B(?=(\d{3})+(?!\d))/g,
          ',',
        );
      }
  ...
  _setCreateCheckedItemsCheque = async (index, key, value) => {
    ...
    if (key === 'chequeNo') {
      ...
      if (value) {
        ...
        let payItems = [0, 0, 0];
        const VPH_PAY_AMT = parseFloat(
          this.props.outstandingBalance.process.header.VPH_PAY_AMT,
        );

        payItems[0] = parseFloat(
          checkedItems.cheques[0].pay ? checkedItems.cheques[0].pay : 0,
        );
        payItems[1] = parseFloat(
          checkedItems.cheques[1].pay ? checkedItems.cheques[1].pay : 0,
        );
        payItems[2] = parseFloat(
          checkedItems.cheques[2].pay ? checkedItems.cheques[2].pay : 0,
        );
