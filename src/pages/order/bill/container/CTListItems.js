...
import { billClearListItems, billSearchListItems, setError } from '../../../../action/bill';
import { addProduct, loadOrderFileByById, orderCancel, orderUpdate, setHeader, setInitialState, setOrderItems } from '../../../../action/order';
...
import { convertProductItemLastBillToOrderItem, generateHeaderForUpdate } from '../../../../utils/Order';
...
class CTListItems extends Component {
  ...
  _printBill = async item => {
    try {
      if (item.DOCINFO?.DI_ACTIVE == 0) {
        ...
        const result = await this.props.loadOrderFileByById(item.DI_REF, moment(item.DI_DATE).format('DDMMYYYY'));
        const {
          RESULT_DATA,
          RESPONSE_DATETIME
        } = result;
        ...
        this._printReceipt(HEADER, ITEMS_PRT, calculateOrderProductProcessedSummary(HEADER, ITEMS), this.props.customer.item.INFO, printTime[0], type, 1);
      }
    } catch (error) {
    ...
    try {
      ...
      if (granted === PermissionsAndroid.RESULTS.GRANTED) {
        if (item.DOCINFO?.DI_ACTIVE == 0) {
          ...
          const html = printReceipt(HEADER, ITEMS_PRT, calculateOrderProductProcessedSummary(HEADER, ITEMS), userToken.VANCONFIG, userToken.COMPANYINFO, this.props.customer.item.INFO, printTime[0], type, 1, isDiscountBath);
          let options = {
            html: html,
    ...
    try {
      if (this.state.userToken.VANCONFIG?.VANCNF_EDIT_AFPRT == 2) {
        if (item.DOCINFO?.DI_ACTIVE == 0) {
          ...
          const data = generateHeaderForUpdate(this.props.customer.item.INFO, ORDER_VIEW.HEADER, type, subType);
          await this.props.setInitialState();
          await this.props.setHeader(data);
          await this.props.setOrderItems(convertProductItemLastBillToOrderItem(EDIT_ITEMS));
  ...
  _printReceipt = async (headerProcessed, productListItemsPRTProcessed, orderProductSummaryProcessed, customer, printTime, type, printTimes) => {
    if (this.props.bluetooth.state == 'connected') {
      const userToken = await getUserToken();
      BplusPrinting.printReceipt(headerProcessed, productListItemsPRTProcessed, orderProductSummaryProcessed, userToken.VANCONFIG, userToken.COMPANYINFO, customer, printTime, type, printTimes);
    }
  };
  _removeConfirmDialog = item => Alert.alert('ประกาศ', ' ยืนยันการยกเลิกเอกสาร', [{
    text: 'ยกเลิก',
    onPress: () => {},
    style: 'cancel'
  }, {
    text: 'ยืนยัน',
    onPress: () => this._orderCancel(item)
  }], {
    cancelable: false
  });
  _printConfirmDialog = item => {
    Alert.alert('ประกาศ', 'ต้องการพิมพ์', [{
      text: 'ยกเลิก',
      onPress: () => {},
      style: 'cancel'
    }, {
      text: 'ยืนยัน',
      onPress: () => this.props.bluetooth.printingType === 'BLUETOOTH' ? this._printBill(item) : this._printBillPDF(item)
    }], {
      cancelable: false
    });
  };
  ...
  _onScroll = event => {
    ...
    currentOffset >= maxOffset && !this.props.bill.isLoading ? this.props.billSearchListItems(true) : null;
  };
  _header = () => {
  ...
  _showPrinterIcon = item => {
    ...
    if (this.state.userToken.VANCONFIG.VANCNF_FRM_ALLCONFIG !== 2 && this.state.userToken.VANCONFIG.VANCNF_FRM_ALLCONFIG !== 3) return false;
    if (item.DOCINFO?.DT_PROPERTIES == 302 && this.state.userToken.VANCONFIG.VANCNF_REPRT_CASHSALES != 2) return false;
    if (item.DOCINFO?.DT_PROPERTIES == 307 && this.state.userToken.VANCONFIG.VANCNF_REPRT_INV != 2) return false;
    return true;
  };
  _renderItem = ({
    item
    ...
    return <ListItem title={<View style={{
      flexDirection: 'row'
        ...
        fontSize: hp('1.6%')
              ...
              {moment(item.DOCINFO?.DI_DATE).add(543, 'years').format('DD/MM/YYYY')}
            </Text>
            <Text style={{
        width: 50,
        marginLeft: 5,
        ...
        flexDirection: 'row'
              ...
              {item.DOCINFO?.DI_ACTIVE == 0 && item.CAN_EDIT == true && item.DOCINFO?.DT_PROPERTIES != 348 && item.DOCINFO?.DT_PROPERTIES != 349 && this.state.userToken && this.state.userToken.VANCONFIG ?
        //&&
        ...
        <View style={{
          marginRight: 15
                  ...
                  <Icon name="edit" size={20} type={'antdesign'} color={MainTheme.colorPrimary} underlayColor="transparent" onPress={() => this._orderUpdate(item)} />
                </View> : null}
            </View>
          </View>} containerStyle={[{
      backgroundColor: item.DOCINFO?.DI_ACTIVE == 0 //&& item.CAN_EDIT == true
      ? MainTheme.colorSecondary : '#FFCBA4'
    }, mainDivider]} bottomDivider titleNumberOfLines={1} />;
  };
  render() {
    return <ListItems header={this._header} listItems={this.props.bill.listItems} renderItem={this._renderItem} errorMessage={this.state.errorMessage} onScroll={this._onScroll} refreshing={this.props.bill.isLoading} onRefresh={this._onRefresh} setState={this._setState} isError={this.props.bill.isError && this.props.bill.listItems.length == 0} isNotFound={this.props.bill.isNotFound && this.props.bill.listItems.length == 0} isSnackBarVisible={this.props.bill.isError && this.props.bill.listItems.length > 0} actionHandler={this._actionHandler} isLoading={this.state.isLoading} loadingMessage={this.state.loadingMessage} successMessage={this.state.successMessage} />;
  }
}
