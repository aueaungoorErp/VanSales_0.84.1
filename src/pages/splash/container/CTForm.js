...
import { Alert, AppState, InteractionManager, PermissionsAndroid, Platform } from 'react-native';
...
import { getMasterDataBankAccounts, getMasterDataProvinces, getMasterDataWareLocations, searchMasterDataBankFileList } from '../../../action/masterData';
import { searchProductCateGoryList } from '../../../action/product-category';
import { getVanConfig, getSaleManV3, getVanConfigV3, readCompanyInfoV3, systemCheck2 } from '../../../action/setting';
...
import { canAutoLogin, getCredentials, getSavedUsername } from '../../../services/SecureCredentials';
import { getLoginGuID, getLoginInfo, getSettingConfig, getUserToken, removeUserToken, setLoginGuID, setSettingConfig, setUserToken } from '../../../utils/Token';
...
class CTForm extends React.Component {
  ...
  _checkPermission = async () => {
    const permissions = [PermissionsAndroid.PERMISSIONS.CAMERA, PermissionsAndroid.PERMISSIONS.READ_EXTERNAL_STORAGE, PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE, PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION, PermissionsAndroid.PERMISSIONS.ACCESS_COARSE_LOCATION];
  ...
  _requestStartupPermissions = () => {
    ...
    InteractionManager.runAfterInteractions(async () => {
      ...
      try {
        ...
        const shouldRetry = message.includes('not attached to an Activity') && this._permissionRetryCount < 5;
        this._log('requestStartupPermissions failed', {
          message,
          shouldRetry
        });
        if (!shouldRetry) {
  ...
  _attemptBiometricOnSplash = async () => {
    try {
      ...
      this._log('biometricOnSplash:promptResult', {
  ...
  _isStartupPayloadComplete = payloadState => Boolean(payloadState?.hasLoginGUID && payloadState?.hasCompanyInfo && payloadState?.hasVanConfig && payloadState?.hasVanMachine);
  _isRetryableStartupError = error => {
    const message = String(error?.message ?? error ?? '').trim();
    if (!message) {
      return true;
    }
    if (/timeout of \d+ms exceeded/i.test(message) || /network error/i.test(message) || /not login/i.test(message) || /vanconfig/i.test(message) || /companyinfo/i.test(message) || /salesman/i.test(message) || /incomplete startup payload/i.test(message)) {
      return true;
    }
    if (/user id not found or wrong password/i.test(message) || /บัญชีผู้ใช้ หรือ รหัสผ่านไม่ถูกต้อง/i.test(message) || /duplicateuser/i.test(message) || /เครื่องเกินขีดจำกัด/i.test(message) || /ลงทะเบียนเกินขีดจำกัด/i.test(message) || /609/.test(message)) {
      return false;
    }
    return true;
  };
    ...
    while (this._isMounted && attempt < maxAttempts) {
      ...
      try {
        ...
        if (!retryable) {
          ...
          if (/duplicateuser/i.test(errorMessage) || /เครื่องเกินขีดจำกัด/i.test(errorMessage) || /ลงทะเบียนเกินขีดจำกัด/i.test(errorMessage)) {
            Alert.alert('ไม่สามารถเข้าสู่ระบบได้', 'หมายเลขรถคันนี้ลงทะเบียนเกินขีดจำกัด หรือมีการใช้งานซ้ำกับเครื่องอื่น');
          }
        ...
        await this._setState('errorMessage', `กำลังลองใหม่อัตโนมัติ ครั้งที่ ${attempt + 1}`);
        await this._delay(Math.min(2000 * attempt, 10000));
      }
    }
    if (this._isMounted) {
      await this._setState('titleProgress', 'เข้าสู่ระบบอัตโนมัติไม่สำเร็จ');
      await this._setState('errorMessage', 'เข้าสู่ระบบอัตโนมัติไม่สำเร็จ กรุณาเข้าสู่ระบบอีกครั้ง');
  ...
  _restoreCachedSession = async setting => {
    ...
    if (!cachedVanConfig) {
      this._log('restoreCachedSession:skip', {
        reason: 'no cached VANCONFIG'
      });
      return false;
    }
    await setUserToken({
  ...
  _hasCachedRememberedSession = async (credentials, setting) => {
    const userToken = await getUserToken();
    const result = Boolean(credentials?.username && credentials?.password && (userToken?.VANCONFIG || setting?.VANCONFIG));
  ...
  _prepareData = async () => {
    ...
    const savedUsername = (await getSavedUsername()) || credentials?.username || null;
    const currentBiometricUser = await findPersistedBiometricUser({
      service: loginInfo?.service ?? null,
      USER_CODE: savedUsername
    });
    const hasSettingConfig = Boolean(setting && setting.baseUrl && setting.vanCNFMachine);
    ...
    const userCode = (await getSavedUsername()) || credentials?.username || null;
  ...
  _getVanConfigV3 = async (BPAPUS_GUID, VANCNF_MACHINE) => {
    try {
      this._log('getVanConfigV3:start', {
        BPAPUS_GUID,
        VANCNF_MACHINE
      });
      ...
      if (BPAPUS_GUID && nextVanConfig?.VANCNF_SLMN) {
        try {
          const salesmanResponse = await this.props.getSaleManV3(BPAPUS_GUID, nextVanConfig.VANCNF_SLMN);
          const salesmanData = this._safeJsonParse(salesmanResponse?.ResponseData);
          if (salesmanResponse?.ResponseCode == 200 && salesmanData && salesmanData.RECORD_COUNT != '0') {
  ...
  _getVanConfig = async (vanCNFMachine, configOverride = null, options = {}) => {
    try {
      const {
      ...
      if (!activeConfig.baseUrl || !activeConfig.vanCNFMachine || !activeConfig.USER_CODE || !activeConfig.USER_PASSWORD) {
      ...
      const response2 = await this.props.registerV3(activeConfig.USER_CODE, activeConfig.USER_PASSWORD);
      const {
        ResponseData,
        ResponseCode
      } = response2;
      this._log('getVanConfig:registerResult', {
        ResponseCode
      });
      if (ResponseCode == 200) {
        ...
        await ensureDeviceRegistrationForLogin(activeConfig.vanCNFMachine, responseData.BPAPUS_GUID);
  ...
  _echoError = async error => {
    const msg = error instanceof Error ? error.message : typeof error === 'string' ? error : error != null ? String(error) : null;
    await this._setState('errorMessage', msg ? msg : 'ไม่สามารถเชื่อมต่อระบบได้');
  };
  _refresh = async () => {
  ...
  render() {
    return <Form progress={this.state.progress} titleProgress={this.state.titleProgress}
    // movingUpDown={movingUpDown}
    // vanMoving={this.state.vanMoving}
    errorMessage={this.state.errorMessage} refresh={this._refresh} goToScreen={this._goToScreen} />;
  }
}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    readCompanyInfoV3: (GUID, CMPNY_CODE) => dispatch(readCompanyInfoV3(GUID, CMPNY_CODE)),
    systemCheck2: data => dispatch(systemCheck2(data)),
    searchMasterDataBankFileList: GUID => dispatch(searchMasterDataBankFileList(GUID)),
    searchCustomerTypeList: vanCNFEnabledAllar => dispatch(searchCustomerTypeList(vanCNFEnabledAllar)),
    searchProductCateGoryList: vanCNFEnabledAllic => dispatch(searchProductCateGoryList(vanCNFEnabledAllic)),
    ...
    registerV3: (username, password) => dispatch(registerV3(username, password)),
