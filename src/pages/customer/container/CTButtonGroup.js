...
import { clearCustomerList, closeCustomerAccount, createTempCus, searchCustomerList, searchCustomerNextDestination, setInitialState as setCustomerInitialState, setCustomerTempCus } from '../../../action/customer';
import { setCustomerType } from '../../../action/customer-type';
import { lookupErpV3Api } from '../../../api/bPlusApi';
import { customerAddButtonGroup, customerProfileDetailButtonGroup, MainTheme } from '../../../constant/lov';
import { strings } from '../../../locales/i18n';
import { assessLongdoApiKeyAvailability, getLongdoApiKeyAlertMessage } from '../../../services/longdomap';
...
class CTButtonGroup extends React.Component {
  ...
  _onPress = async item => {
    if (item.methodType === 'function') {
      if (item.methodName === 'order') {
        ...
        await this.props.setCustomerTempCus({
          PROVINCE: null,
          ARC_VAT_TY: null
        });
  ...
  _closeCustomerAccount = async () => {
    try {
      ...
      if (this.state.closeReason === null || this.state.closeReason.trim() === '') {
  ...
  _addCustomer = async () => {
    try {
      ...
      const provinceIndex = this.props.masterData.province.listItems.findIndex(item => item.Id === tempCus.PROVINCE);
      const address3Index = this.props.masterData.district.listItems.findIndex(item => item.Id === tempCus.ADDRESS3);
      const address2Index = this.props.masterData.subDistrict.listItems.findIndex(item => item.Id === tempCus.ADDRESS2);
      const PROVINCE = this.props.masterData.province.listItems[provinceIndex].NameInThai;
      const ADDRESS3 = this.props.masterData.district.listItems[address3Index].NameInThai;
      const ADDRESS2 = this.props.masterData.subDistrict.listItems[address2Index].NameInThai;
      tempCus = {
        ...tempCus,
        ID: 0,
        CODE: '',
        ...
        DOCSTATUS: 0,
        REMARK: '',
        DOCDATE: moment().format('YYYY-MM-DDTHH:mm:SS'),
        //'2019-06-05T03:39:58.537Z',
        PROVINCE: PROVINCE,
        ADDRESS3: ADDRESS3,
        ADDRESS2: ADDRESS2,
        ARC_REMARK: '',
  ...
  _refreshCustomerList = async userToken => {
    ...
    await this.props.setCustomerType({
      ARCAT_KEY: null,
      ARCAT_NAME: null
    });
  ...
  _completeAlertDialog = () => Alert.alert('สำเร็จ', 'สร้างข้อมูลลูกค้าเรียบร้อย', [{
    text: 'ตกลง',
  ...
  _checkProperties = obj => {
    ...
    if (obj.TAXID === '' || obj.TAXID === undefined || obj.TAXID === null || obj.TAXID.length !== 13) {
      return strings('customer.error_tax_id_should_be_13_digits');
    }
    if (obj.PROVINCE === undefined || obj.PROVINCE === null || obj.PROVINCE === '') {
      return strings('customer.error_province_is_required');
    }
    if (obj.ADDRESS3 === undefined || obj.ADDRESS3 === null || obj.ADDRESS3 === '') {
    ...
    if (obj.ADDRESS1 === undefined || obj.ADDRESS1 === null || obj.ADDRESS1 === '') {
      return strings('customer.error_adress1_is_required');
    }
    if (obj.POSTCODE === undefined || obj.POSTCODE === null || obj.POSTCODE === '' || obj.POSTCODE.length !== 5) {
      return strings('customer.error_post_code_should_be_5_digits');
    }
    if (obj.CONTACTNAME === undefined || obj.CONTACTNAME === null || obj.CONTACTNAME === '') {
      return strings('customer.error_contact_name_is_required');
    }
    if (obj.TEL === undefined || obj.TEL === null || obj.TEL === '') {
      return 'กรุณากรอกเบอร์โทรศัพท์';
    }
    if (obj.ARC_NAME === undefined || obj.ARC_NAME === null || obj.ARC_NAME === '') {
      obj.ARC_NAME === ' ';
    }
    if (obj.ARC_PAYMENT_PERIOD === undefined || obj.ARC_PAYMENT_PERIOD === null) {
  ...
  _renderItem = (item, key) => {
    ...
    return <TouchableOpacity key={key} style={[item.containerStyle, {
      flex: 1,
      ...
      backgroundColor: isDisabled ? MainTheme.colorNonary : isPrimary ? MainTheme.colorPrimary : '#FFFFFF'
    }]} onPress={() => {
      this._onPress(item);
        ...
        fontWeight: '700',
        fontSize: 16,
        color: isDisabled ? '#FFFFFF' : isPrimary ? MainTheme.colorSecondary : '#22312B'
      }]}>{item.title}</Text>
            </TouchableOpacity>;
  };
  render() {
    return <ButtonGroup renderItem={this._renderItem} listItems={this.props.screen === 'add' ? customerAddButtonGroup : customerProfileDetailButtonGroup} setState={this._setState} errorMessage={this.state.errorMessage} isLoading={this.props.masterData.isLoading} loadingMessage={this.state.loadingMessage} isOpenReasonDialog={this.state.isOpenReasonDialog} closeReason={this.state.closeReason} closeCustomerAccount={this._closeCustomerAccount} successCloseReasonMessage={this.state.successCloseReasonMessage} errorCloseReasonMessage={this.state.errorCloseReasonMessage} isCloseReasonLoading={this.state.isCloseReasonLoading} isOpenSuccessCloseReasonDialog={this.state.isOpenSuccessCloseReasonDialog} />;
  }
}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    searchCustomerNextDestination: () => dispatch(searchCustomerNextDestination()),
