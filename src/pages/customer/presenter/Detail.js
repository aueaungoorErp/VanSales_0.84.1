        ...
        <View style={styles.slide2}>
          <View>
            <Text style={styles.content} allowFontScaling={false}>
              ...
              {AR_SUMMARY && AR_SUMMARY.LST_VISIT_DOC ? toBuddhistYear(AR_SUMMARY.LST_VISIT_DOC, '-', 2) : null}
            </Text>
            <Text style={styles.content} allowFontScaling={false}>
              {strings('customer.last_sell_date')}{' '}
              {AR_SUMMARY && AR_SUMMARY.LAST_DI_DATE ? toBuddhistYear(AR_SUMMARY.LAST_DI_DATE, '-', 2) : null}
            </Text>
            <Text style={styles.content} allowFontScaling={false}>
              {strings('customer.last_bill_no')}{' '}
              {AR_SUMMARY && AR_SUMMARY.LAST_DI_REF ? AR_SUMMARY.LAST_DI_REF : null}
            </Text>
            <Text style={styles.content} allowFontScaling={false}>
              {strings('customer.last_sell_amount')}{' '}
              {AR_SUMMARY && AR_SUMMARY.LAST_DI_AMT ? parseFloat(AR_SUMMARY.LAST_DI_AMT).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',') : null}
            </Text>
            <Text style={styles.content} allowFontScaling={false}>
              {strings('customer.credit_limit')}{' '}
              {AR_SUMMARY && AR_SUMMARY.ARS_CRE_LIM ? parseFloat(AR_SUMMARY.ARS_CRE_LIM).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',') + ' ' : null}
            </Text>
            <Text style={styles.content} allowFontScaling={false}>
              {strings('customer.credit_balance')}{' '}
              {CREDIT_LIM === 1 || CREDIT_LIM === 2 ? AR_SUMMARY && (AR_SUMMARY.ARS_CRE_LIM === '0' ? '0.00' : AR_SUMMARY.ARS_CRE_REMAIN_NPDC) ?
            ...
            // :
            parseFloat(AR_SUMMARY.ARS_CRE_LIM === '0' ? '0' : AR_SUMMARY.ARS_CRE_REMAIN_NPDC).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',') + ' ' : null : null}
            ...
            <Text style={styles.content} allowFontScaling={false}>
              ยอดหนี้คงค้าง{' '}
              {AR_SUMMARY && AR_SUMMARY.SUM_ARD_NET ? parseFloat(AR_SUMMARY.SUM_ARD_NET).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',') + ' ' : null}
            </Text>
          </View>
        </View>
      </Swiper>
