...
class CTButtonGroup extends Component {
  ...
  componentDidUpdate = prevProps => {
    ...
    if (this.state.showDialog && currentLat !== null && currentLng !== null && (prevLat !== currentLat || prevLng !== currentLng)) {
  ...
  _renderItem = (item, key) => {
    ...
    const isDisabled = item.title === 'ยกเลิก' && this.state.userToken && this.state.userToken.VANCONFIG && this.state.userToken.VANCONFIG.VANCNF_FORCE_GPS === 1 && previousRoute.name === 'OrderScreen' || item.title === 'เช็คอิน' && position.latitude === null && position.longitude === null;
    return <TouchableOpacity key={key} style={[styles.button, item.buttonStyle || {}, item.containerStyle || {}, isDisabled ? styles.disabledButton : null]} onPress={() => {
      this._onPress(item);
    }} disabled={isDisabled}>
  ...
  _onPress = async item => {
    ...
    if (item.methodName === 'confirm') {
      if (await this._validateForm()) {
        const position = this.props.geolocation && this.props.geolocation.position ? this.props.geolocation.position : {};
        if (userToken && userToken.VANCONFIG && userToken.VANCONFIG.VANCNF_WARN_NOGPS == 2 && position.latitude === null && position.longitude === null) {
          this._setState('showDialog', true);
        }
        if (routeName === 'OrderScreen' || routeName === 'CustomerProfileDetail') Navigator.pop(1, true);
  ...
  _validateForm = async () => {
    try {
      var ret = false;
      if (this.state.userToken.VANCNF_RANGECHECKIN !== null && this.state.userToken !== 0) {
        if (!this.props.customer.item || !this.props.customer.item.INFO || isNaN(parseFloat(this.props.customer.item.INFO.ADDB_GPS_LAT_S)) || isNaN(parseFloat(this.props.customer.item.INFO.ADDB_GPS_LONG_S))) {
          ...
          const AsyncAlert = async () => new Promise(resolve => {
            Alert.alert('คำเตือน', 'ต้องการที่จะบันทึกพิกัดปัจจุบันสำหรับลูกค้ารายนี้ หรือไม่ ? \r\n', [{
              text: 'บันทึก',
              onPress: () => {
                resolve('YES');
              }
          ...
          if (x) {
            ...
            const bodyRequest = {
              ...
              'BPAPUS-PARAM': '{"ADDB_KEY":  "' + this.props.customer.item.INFO.ADDB_KEY + '","ADDB_GPS_LAT_S": ' + this.props.geolocation.position.latitude + ',"ADDB_GPS_LONG_S": ' + this.props.geolocation.position.longitude + '}',
              'BPAPUS-FILTER': '',
              'BPAPUS-ORDERBY': '',
              'BPAPUS-OFFSET': '0',
              'BPAPUS-FETCH': '0'
        ...
        if (cancheck) {
          ...
          const AsyncAlert = async () => new Promise(resolve => {
            Alert.alert('เช็คอิน', 'ทำรายการเช็คอินสำเร็จ', [{
              text: 'ตกลง',
              onPress: () => {
                resolve('YES');
              }
      ...
      this._setState('isLoading', false);
      const AsyncAlert = async () => new Promise(resolve => {
        Alert.alert('เช็คอินไม่สำเร็จ', typeof error === 'string' ? error : error && error.message ? error.message : 'เกิดข้อผิดพลาด', [
  ...
  render() {
    ...
    const canSkip = this.state.userToken && this.state.userToken.VANCONFIG && this.state.userToken.VANCONFIG.VANCNF_WARN_NOGPS === 1;
    return <View>
        <Modal transparent visible={this.state.showDialog} animationType="fade" onRequestClose={() => this._setState('showDialog', false)}>
          <View style={styles.modalOverlay}>
            <View style={styles.dialogCard}>
              <Text style={styles.dialogTitle}>คำเตือน</Text>
              <Text style={styles.dialogMessage}>{geolocationMessage}</Text>
              <View style={styles.dialogButtonRow}>
                <TouchableOpacity style={[styles.dialogButton, styles.primaryDialogButton]} onPress={isPermissionBlocked ? this._openLocationSettings : this._getCurrentPosition}>
                  <Text style={styles.primaryDialogButtonText}>
                    {isPermissionBlocked ? 'เปิดตั้งค่า' : 'ลองใหม่'}
                  </Text>
                </TouchableOpacity>
                {canSkip ? <TouchableOpacity style={[styles.dialogButton, styles.secondaryDialogButton]} onPress={this._onSkip}>
        ...
        <Modal transparent visible={this.state.errorMessage !== ''} animationType="fade" onRequestClose={() => this._setState('errorMessage', '')}>
          <View style={styles.modalOverlay}>
            <View style={styles.dialogCard}>
              <Text style={styles.dialogTitle}>คำเตือน</Text>
              ...
              <View style={styles.dialogButtonRow}>
                <TouchableOpacity style={[styles.dialogButton, styles.primaryDialogButton]} onPress={() => this._setState('errorMessage', '')}>
        ...
        <Modal transparent visible={this.state.isLoading} animationType="fade" onRequestClose={() => null}>
          <View style={styles.modalOverlay}>
        ...
        <ButtonGroup errorMessage={this.state.errorMessage} renderItem={this._renderItem} buttonListItems={checkInFormButtonGroup} />
      </View>;
  }
}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    checkDistance: (arKey, lat, lnt, range) => dispatch(checkDistance(arKey, lat, lnt, range))
  };
};
...
const ConnectedCTButtonGroup = connect(mapStateToProps, mapDispatchToProps)(CTButtonGroup);
const CTButtonGroupWithMutation = props => {
  const updateVanPositionMutation = useUpdateVanPosition();
  return <ConnectedCTButtonGroup {...props} updateVanPosition={updateVanPositionMutation.mutateAsync} />;
};
