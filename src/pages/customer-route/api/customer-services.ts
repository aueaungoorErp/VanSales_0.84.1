...
const formatArrayResponseForLog = (payload: any) => {
  ...
  return {
    ...
    ResponseData: {
      ...
      currentRowCount: Array.isArray(responseData[arrayKey]) ? responseData[arrayKey].length : 0,
      arrayKey,
...
const buildArCodeInFilter = (arCodes: Array<string | number>) => {
  const normalizedCodes = arCodes.map(item => String(item ?? '').trim()).filter(Boolean);
  if (normalizedCodes.length === 0) {
    return '';
  }
  return `and AR_CODE in (${normalizedCodes.map(item => `'${item}'`).join(',')})`;
};
export const customerRouteBatchDetails = async (params: CustomerRouteBatchDetailParams) => {
