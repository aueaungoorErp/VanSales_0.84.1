...
class CTDetailFormSCR extends Component {
  ...
  _processOrderItem = async () => {
    try {
      ...
      if (this._validateItem()) {
        const response = await this.props.processOrderItemSCR(genenrateOrderForProcessToServerSCR(this.props.order, convertProductItemToOrderItemSCR(this.props.product.scrChooseItems.filter(value => value != null), this.props.order.header.AR_ORDER_TYPE === ORDER_TYPE_RETURN ? true : false), this.props.order.header.AR_ORDER_TYPE === ORDER_TYPE_RETURN ? true : false, this.props.order.header.AR_ORDER_TYPE === ORDER_TYPE_RETURN ? 1 : 0));
        const {
          RESULT_DATA,
          STATUS,
          ERROR_MESSAGES
  ...
  _renderItem = (item, key) => {
    ...
    }, item.methodName === 'confirm' ? this.props.product.disabledButton : this.state.disabledButton ? {
      backgroundColor: MainTheme.colorNonary
    } : null]} onPress={() => {
      this._onPress(item);
    }} disabled={item.methodName === 'confirm' ? this.props.product.disabledButton : this.state.disabledButton} activeOpacity={0.7}>
              <Text style={item.titleStyle}>{item.title}</Text>
            </TouchableOpacity>;
  };
  render() {
    return <ButtonGroupSCR renderItem={this._renderItem} listItems={productDetailFormButtonGroup} successMessage={this.state.successMessage} errorMessage={this.state.errorMessage} isLoading={this.state.isLoading} />;
  }
}
