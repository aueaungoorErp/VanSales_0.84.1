...
import { clearItem, processOrderItem, searchProductByGoodsCode, setGoodsCodeCriteria, setInitialState, setItemDiscount, setItemFree, setItemLot, setItemNetPrice, setItemQty, setItemSerial, setItemTotalDiscount, setItemTotalPrice } from '../../../action/product';
import { MainTheme, productDetailFormButtonGroup } from '../../../constant/lov';
import Navigator from '../../../services/Navigator';
import { calculateDiscount, calculateOrderProductTotalDiscount, discountFormat, numberOnly } from '../../../utils/Culculate';
import { getUserToken } from '../../../utils/Token';
import DetailForm from '../presenter/DetailForm';
class CTDetailForm extends Component {
  _isMounted = false;
  _scanBarcodeEnabled = true;
  _isTransferOrder = () => this.props.order.header.AR_ORDER_TYPE === 'โอนย้ายสินค้า';
  constructor(props) {
    super(props);
    this.state = {
      isLoading: false,
  ...
  _getTransferNegativeBalanceMessage = () => {
    ...
    const preventNegativeBalance = this.state.userToken?.VANCONFIG?.VANCNF_NOV_SKU_BAL === 1;
    ...
    if (isTransferOrder && preventNegativeBalance && hasSelectedGoods && Number.isFinite(goodsBalance) && goodsBalance < 0) {
  ...
  _getNormalizedItemQty = item => {
    ...
    const shouldConvertByPack = totalPrice === null || totalPrice === '' || parseInt(totalPrice) <= 0;
  ...
  _renderItem = (item, key) => {
    ...
    const isBlockedAction = hasTransferNegativeBalance && item.methodName === 'confirm';
    const disabled = item.methodName === 'confirm' ? (isTransferOrder ? false : this.state.submitDisabled) || isBlockedAction : this.state.disabledButton;
    return <>
  ...
  _onSearchByGoodsCode = async () => {
    try {
      if (this.props.product.criteria.GOODS_CODE !== null && this.props.product.criteria.GOODS_CODE !== '') {
        this._setIsLoading(true);
        this._setErrorMessage(null);
        this._setSuccessMessage(null);
        this.props.clearItem();
  ...
  _validateItem = () => {
    ...
    if (GOODS_QTY !== null && GOODS_QTY !== '' && parseInt(GOODS_QTY) <= 0 && GOODS_DISCOUNT !== null && GOODS_DISCOUNT !== '' && parseInt(GOODS_DISCOUNT) > 0) {
      this._setErrorMessage('ไม่สามารถใส่ส่วนลดได้เนื่องจากจำนวนสินค้าเป็น 0');
      return false;
    }
    if (GOODS_QTY !== null && GOODS_QTY !== '' && parseInt(GOODS_QTY) >= 0 && GOODS_DISCOUNT !== null && GOODS_DISCOUNT !== '' && parseInt(GOODS_DISCOUNT) > 0 && GOODS_DISCOUNT * GOODS_QTY >= ARPLU_U_PRC * GOODS_QTY) {
      this._setErrorMessage('ไม่สามารถใส่ส่วนลดได้ ส่วนลดมากกว่าราคาขาย');
      return false;
    }
    if ((GOODS_QTY === null || GOODS_QTY === '' || parseInt(GOODS_QTY) <= 0) && (GOODS_FREE === null || GOODS_FREE === '' || parseInt(GOODS_FREE) <= 0)) {
    ...
    itemdigit = (Math.abs(good_inVan_qty).toFixed(0) - Math.floor(Math.abs(good_inVan_qty) / UTQ_QTY).toFixed(0) * UTQ_QTY).toFixed(0);
    itemdigit = UTQ_QTY == '1' ? '' : '.' + itemdigit.padStart(('' + UTQ_QTY).length, '0');
    if (GOODS_TOTAL_PRC === null || GOODS_TOTAL_PRC === '' || parseInt(GOODS_TOTAL_PRC) <= 0) {
      //this.props.product.item.GOODS_QTY = GOODS_QTY * UTQ_QTY;
      convert_GOODS_QTY_remain = Math.floor((parseFloat(GOODS_QTY) || 0) * UTQ_QTY);
      convert_GOODS_FREE_remain = Math.floor((parseFloat(GOODS_FREE) || 0) * UTQ_QTY);
    } else {
    ...
    const availableQtyAfterReserved = parseInt(good_inVan_qty) - parseInt(reservedQtyForCurrentGoods);
    if (this.state.userToken.VANCONFIG.VANCNF_NOV_SKU_BAL === 1) if (this.props.order.header.AR_ORDER_TYPE === 'ขายสินค้า') {
      {
        if (availableQtyAfterReserved - parseInt(convert_GOODS_QTY_remain + convert_GOODS_FREE_remain) < 0) {
          this._setErrorMessage('สินค้าในรถมีไม่พอขาย จำนวนคงเหลือ : ' + parseFloat(itemremain + itemdigit) + ' (' + UTQ_NAME + ')');
          return false;
        }
      }
    }
  ...
  _calculateTotalPrice() {
    const uPrc = this.props.product.item.ARPLU_U_PRC ? this.props.product.item.ARPLU_U_PRC : 0;
    const qty = this.props.product.item.GOODS_QTY ? this.props.product.item.GOODS_QTY : 0;
    return Math.round(uPrc * qty * 100) / 100;
  }
  _calculateNetPrice() {
    const totalPrice = this.props.product.item.GOODS_TOTAL_PRC ? this.props.product.item.GOODS_TOTAL_PRC : 0;
    const totalDiscount = this.props.product.item.GOODS_TOTAL_DISCOUNT ? this.props.product.item.GOODS_TOTAL_DISCOUNT : 0;
    return Math.round((totalPrice - totalDiscount) * 100) / 100;
  }
  _calculateDiscountPerUnit() {
    let uPrc = this.props.product.item.ARPLU_U_PRC ? this.props.product.item.ARPLU_U_PRC : 0;
    const discountInput = this.props.product.item.GOODS_DISCOUNT ? this.props.product.item.GOODS_DISCOUNT.split(/[,+]/) : '';
  ...
  _onSubmitEditing = (discount, DI_AMOUNT) => {
    if (discount) {
      const discountRGEX = /^[0-9*]+\.?[0-9*]*[bB%*]?([,+\-\/]?([0-9*]+\.?[0-9*]*[bB%*]?)?)*$/;
      const discountFormat = discountRGEX.test(discount);
      const discountPER = /^[0-9*]?([*]?([0-9*]+\.?[0-9*]*[%]?)?)([0-9*]+\.?)*$/;
  ...
  render() {
    return <DetailForm item={this.props.product.item} goodsCode={this.props.product.criteria.GOODS_CODE} buttonListItems={productDetailFormButtonGroup} renderItem={this._renderItem} setItemQty={this._setItemQty.bind(this)} setItemLot={this._setItemLot.bind(this)} setItemSerial={this._setItemSerial.bind(this)} setItemDiscount={this._setItemDiscount.bind(this)} setItemFree={this._setItemFree.bind(this)} errorMessage={this.state.errorMessage} isLoading={this.state.isLoading} getRef={this._setTextInputQtyRef} onSearch={this._onSearchByGoodsCode} setGoodsCode={this._setGoodsCode} goodsCodeEditable={this.props.goodsCodeEditable} orderType={this.props.order.header.AR_ORDER_TYPE} editableItemFree={this.state.userToken && this.state.userToken.VANCONFIG && this.state.userToken.VANCONFIG.VANCNF_ENABLE_QFREE && this.state.userToken.VANCONFIG.VANCNF_ENABLE_QFREE == 2 ? true : false} editableItemDiscount={this.state.userToken && this.state.userToken.VANCONFIG && this.state.userToken.VANCONFIG.VANCNF_ENABLE_UDSC && this.state.userToken.VANCONFIG.VANCNF_ENABLE_UDSC == 2 ? true : false} onScanBarcodePress={this._onScanBarcodePress} onSubmitEditing={this._onSubmitEditing} />;
  }
}
