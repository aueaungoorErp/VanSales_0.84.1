...
class CTDetailForm extends Component {
  ...
  _validateItem = () => {
    ...
    if (
      ...
      console.log('this.props.product.item.GOODS_QTY >> ', this.props.product.item.GOODS_QTY)
      console.log('GOODS_QTY >> ', GOODS_QTY)


      if (
        (GOODS_TOTAL_PRC === null || GOODS_TOTAL_PRC === '' || parseInt(GOODS_TOTAL_PRC) <= 0)) {
      ...
      console.log('this.props.product.item.GOODS_QTY >> 2', this.props.product.item.GOODS_QTY)
      ...
      if (this.state.userToken.VANCONFIG.VANCNF_NOV_SKU_BAL === 1)
        if (this.props.order.header.AR_ORDER_TYPE === 'ขายสินค้า') {
          ...
          {
            if (parseInt(good_inVan_qty) - parseInt(convert_GOODS_QTY_remain) < 0) {
