...
import { setInitialState, mileCheckinCreate, mileAttachImage } from '../../../action/mile';
...
class CTButtonGroup extends Component {
  ...
  _renderItem = (item, key) => {
    return <TouchableOpacity key={key} style={[styles.button, item.buttonStyle || {}, item.containerStyle || {}, this.state.isLoading ? styles.disabledButton : null]} disabled={this.state.isLoading} onPress={() => {
      this._onPress(item);
    }}>
  ...
  _onSubmit = async () => {
    try {
      if (this._validateForm()) {
        ...
        await this.props.mileAttachImage(genenrateMileAttachImageToServer(this.props.mile.item.photo));
  ...
  _validateForm = () => {
    ...
    if (this.props.mile.item.mileage === null || this.props.mile.item.mileage === '') {
  ...
  render() {
    return <AndroidBackHandler onBackPress={this._onBackButtonPressAndroid}>
        <ButtonGroup errorMessage={this.state.errorMessage} renderItem={this._renderItem} buttonListItems={mileFormButtonGroup} isLoading={this.state.isLoading} />
      </AndroidBackHandler>;
  }
}
