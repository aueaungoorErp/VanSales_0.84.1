...
import { ActivityIndicator, Alert, Animated, Dimensions, Image, Keyboard, Modal, Pressable, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
import { hydrateUserBiometricState, registerV3, setNewUser } from '../../../action/user';
import { getVanConfigV3, readCompanyInfoV3, systemCheck2 } from '../../../action/setting';
...
import { fetchLoginData, performLogin, type LoginDeps, type ServiceSetting, type UserLogin } from '../action/loginActions';
const AntDesign = require('react-native-vector-icons/AntDesign').default as ComponentType<any>;
const MaterialCommunityIcons = require('react-native-vector-icons/MaterialCommunityIcons').default as ComponentType<any>;
...
const mapDispatchToProps = (dispatch: any) => {
  return {
    getVanConfigV3: (VANCNF_MACHINE: string) => dispatch(getVanConfigV3(VANCNF_MACHINE)),
    registerV3: (username: string, password: string) => dispatch(registerV3(username, password)),
    systemCheck2: (data: any) => dispatch(systemCheck2(data)),
    readCompanyInfoV3: (GUID: string, CMPNY_CODE: number) => dispatch(readCompanyInfoV3(GUID, CMPNY_CODE)),
    searchCustomerTypeList: (vanCNFEnabledAllar: any) => dispatch(searchCustomerTypeList(vanCNFEnabledAllar)),
    searchProductCateGoryList: (vanCNFEnabledAllic: any) => dispatch(searchProductCateGoryList(vanCNFEnabledAllic)),
...
const Form: React.FC<FormProps> = props => {
  ...
  const [listServiceSettings, setListServiceSettings] = useState<ServiceSetting[]>([]);
  ...
  const updateUserLogin = useEffectEvent((nextState: React.SetStateAction<UserLogin>) => {
    setUserLogin(previousState => {
      const resolvedState = typeof nextState === 'function' ? (nextState as (prevState: UserLogin) => UserLogin)(previousState) : nextState;
  ...
  const fetchData = useEffectEvent(async () => {
    ...
    try {
      ...
      const userEditedDuringFetch = isEditingRef.current || editVersionRef.current !== editVersionAtStart;
  ...
  const onLogin = () => performLogin(userLoginRef.current, isRememberPassword, props, {
    setErrorMessage,
    setIsLoading,
  ...
  const handleKeyboardDidShow = useEffectEvent((event?: {
    duration?: number;
  }) => {
  ...
  const onFingerprintLogin = async () => {
    ...
    try {
      ...
      if (!sensorInfo?.available) {
        Alert.alert('ไม่สามารถสแกนนิ้วได้', 'อุปกรณ์นี้ยังไม่รองรับหรือยังไม่ได้ตั้งค่า Biometrics');
        return;
      }
      ...
      const service = fingerUser.service ?? userLoginRef.current.service ?? listServiceSettings?.[0]?.value ?? null;
      const loginPayload = {
        service,
  ...
  const onChangeService = async (value: string | null, listOverride: ServiceSetting[] | null = null, autoContinue = false) => {
    if (!value) {
      return;
    }
    isEditingRef.current = true;
    editVersionRef.current += 1;
    const list = Array.isArray(listOverride) ? listOverride : listServiceSettings;
    updateUserLogin(oldState => ({
      ...oldState,
      service: value
    }));
    const currentUserLogin = userLoginRef.current;
    const serviceItem = list.find(item => item.value === value) || list.find(item => item.number === value) || null;
    const finalBaseUrl = serviceItem?.webURL ?? serviceItem?.baseUrl ?? serviceItem?.baseURL ?? '';
    const resolvedUserCode = currentUserLogin.USER_CODE ?? serviceItem?.USER_CODE ?? '';
    const currentPassword = currentUserLogin.USER_PASSWORD ?? '';
    const resolvedPassword = String(currentPassword).trim() ? currentPassword : '';
    if (!String(currentUserLogin.USER_CODE ?? '').trim() && serviceItem?.USER_CODE) {
      updateUserLogin(oldState => ({
        ...oldState,
    ...
    if (autoContinue && String(resolvedUserCode).trim() && String(resolvedPassword).trim()) {
      void onLogin();
    }
  };
  ...
  return <View style={styles.container}>
      <View style={styles.settingSection}>
        <Pressable hitSlop={10} onPress={() => Navigator.navigate('LoginSetting')}>
          <AntDesign name="setting" color={MainTheme.colorSecondary} size={24} />
        </Pressable>
      </View>
      ...
      transform: [{
        ...
        <View style={styles.formWidth}>
          <View style={styles.hiddenServiceContainer}>
            <Image style={styles.leftIcon} resizeMode="contain" source={require('../../../images/person.png')} />

            <View style={styles.pickerWrapper}>
              <RNPickerSelect onValueChange={value => {
              onChangeService(value);
            }} items={listServiceSettings} value={userLogin.service} style={{
              iconContainer: {
                top: 10,
                right: 0
              },
              ...
              label: 'เลือก',
              value: null
            }} useNativeAndroidPickerStyle={false} textInputProps={{
              underlineColorAndroid: 'transparent'
            }} Icon={() => <AntDesign name="down" size={20} color={MainTheme.colorPrimary} style={{
              marginTop: 2
            }} />} />
            </View>
          </View>
          ...
          <View style={styles.inputRow}>
            ...
            <TextInput placeholder={strings('login.username')} placeholderTextColor={MainTheme.placeholerTextInput} value={toInputValue(userLogin.USER_CODE)} autoCapitalize="characters" style={styles.textInput} underlineColorAndroid="transparent" onFocus={animateHeaderUp} onChangeText={value => {
            isEditingRef.current = true;
            editVersionRef.current += 1;
            updateUserLogin(oldState => ({
              ...oldState,
          ...
          <View style={styles.inputRow}>
            <Image style={styles.leftIcon} resizeMode="contain" source={require('../../../images/lock.png')} />

            <TextInput placeholder={strings('login.password')} secureTextEntry={isShow} placeholderTextColor={MainTheme.placeholerTextInput} value={toInputValue(userLogin.USER_PASSWORD)} autoComplete="off" textContentType="none" importantForAutofill="no" autoCorrect={false} style={styles.textInput} underlineColorAndroid="transparent" onFocus={animateHeaderUp} onChangeText={handleChangePassword} />

            <MaterialCommunityIcons name={isShow ? 'eye-off-outline' : 'eye-outline'} size={22} color={MainTheme.colorTertiary} onPress={() => {
            setIsShow(old => !old);
          }} />
          </View>

          <View style={styles.rememberRow}>
            <Pressable accessibilityRole="checkbox" accessibilityLabel="remember password" accessibilityState={{
            checked: !!isRememberPassword
          }} onPress={() => {
            onToggleRememberPassword();
          }} style={styles.checkboxPressable}>
              <View style={[styles.checkboxBox, isRememberPassword ? styles.checkboxBoxChecked : null]}>
                {isRememberPassword ? <AntDesign name="check" size={14} color="#ffffff" /> : null}
              </View>
            </Pressable>
          ...
          <View style={styles.loginButtonWrap}>
            <View style={styles.loginButtonRow}>
              ...
              {fingerUser?.USER_CODE ? <TouchableOpacity style={styles.fingerprintButton} onPress={() => void onFingerprintLogin()} activeOpacity={0.7}>
                  <MaterialCommunityIcons name="fingerprint" size={32} color={MainTheme.colorTertiary} />
      ...
      <Modal transparent visible={!!isLoading || !!errorMessage} animationType="fade">
        <View style={styles.loadingBackdrop}>
          {!!isLoading ? <View style={styles.loadingModalCard}>
              <View style={styles.loadingInline}>
                <ActivityIndicator size="large" color={MainTheme.colorTertiary} style={styles.loadingSpinner} />
                <Text style={styles.loadingText}>กำลังเข้าสู่ระบบ</Text>
              </View>
            </View> : <View style={styles.errorModalCard}>
              <Text style={styles.errorTitle}>ไม่สามารถเข้าสู่ระบบได้</Text>
              <Text style={styles.errorBody}>{errorMessage}</Text>
              <TouchableOpacity style={styles.errorButton} onPress={() => setErrorMessage(null)}>
