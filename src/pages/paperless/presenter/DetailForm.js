...
import { GetPrintReport, downloadReport, getPrintStatus, getReportName } from '../../../action/report';
...
const Detail = props => {
  ...
  const [GETREPORTNAME, setGETREPORTNAME] = useState(reportReducer.dataReportV3);
  useEffect(() => {
    if (countdown != -1) {
  ...
  const _onItemPress = async item => {
    ...
    if (FROM > TO) {
      ...
      const res = await dispatch(GetPrintReport(item.RPTSVR_GUID, conDateFrom, conDateTo));
  ...
  const _getStatus = async RPTQUE_GUID => {
    ...
    if (res2.RPTQUE_RSLT_STATUS == 0) {
      ...
      Alert.alert(res2.RPTSVR_NAME, res2.SYSLKUP_T_DESC, [{
        text: 'ตกลง',
        onPress: () => {}
      }], {
        cancelable: false
    ...
    return <ListItem title={<View style={{
      flexDirection: 'row'
        ...
        flexDirection: 'column'
          ...
          </View>} hideChevron onPress={() => {
      _onItemPress(item);
    }} containerStyle={mainDivider} bottomDivider />;
  };
  return <View style={styles.container}>
      <ListItems renderItem={_renderItem} onRefresh={_onRefresh} onScroll={_onScroll} actionHandler={_actionHandler} listItems={reportReducer.dataReportV3 ? reportReducer.dataReportV3 : GETREPORTNAME}
    // isNotFound={
    //   this._campaignCode !== null
    //     ? this.props.campaignARCPGNType.isNotFound
    //     : this.props.campaignType.isNotFound
    // }
    errorMessage={reportReducer.errorMessage} isError={reportReducer.isError}
    // isSnackBarVisible={this.state.isSnackBarVisible}
    onButtonPress={_onReload} isLoading={reportReducer.isLoading} paperlessErrorMessage={reportReducer.errorMessage} />
