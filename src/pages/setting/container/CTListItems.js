...
import { Alert, Image, PermissionsAndroid, Platform, StyleSheet, Switch, Text, TouchableOpacity, View } from 'react-native';
...
class CTListItems extends React.Component {
  ...
  componentDidMount() {
    ...
    this.focusSubscription = this.props.navigation?.addListener('focus', this._handleFocus);
  }
  ...
  _checkPermission = async () => {
    permissions = [PermissionsAndroid.PERMISSIONS.CAMERA, PermissionsAndroid.PERMISSIONS.READ_EXTERNAL_STORAGE, PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE];
  ...
  _loadBBLQrPaymentSetting = async () => {
    const [storedEnabled, bblBaseUrl] = await Promise.all([getBBLQrPaymentEnabled(), getBBLPaymentBaseUrl()]);
    this.setState({
      bblQrPaymentEnabled: storedEnabled === null ? !!String(bblBaseUrl || '').trim() : !!storedEnabled
    });
  };
  ...
  _canPressItem = item => item.methodType === 'new-page' || item.methodType === 'function' && item.methodName !== 'toggleBiometrics' && item.methodName !== 'toggleBBLQrPayment';
  ...
  _getListItems = () => {
    ...
    const manualIndex = settingListItems.findIndex(item => item.screen === 'Manual');
    if (manualIndex < 0) {
      return [...settingListItems, bblQrPaymentItem, longdoApiKeyItem, biometricItem];
    }
    return [...settingListItems.slice(0, manualIndex), bblQrPaymentItem, longdoApiKeyItem, biometricItem, ...settingListItems.slice(manualIndex)];
  };
  _renderItem = ({
    item
  }, key) => {
  ...
  _renderCustomPattern = item => {
    ...
    return <TouchableOpacity style={itemStyles.row} onPress={() => this._onPress(item)} activeOpacity={0.6}>
        <View style={itemStyles.iconContainer}>
          {isPDF ? <FontAwesome5 name="file-pdf" color={MainTheme.colorPrimary} size={20} solid /> : <Image style={{
          width: 25,
          height: 25
        }} resizeMode="contain" source={isConnected ? item.imgSrc : item.subImgSrc} />}
        </View>
        <View style={itemStyles.textContainer}>
          <Text style={itemStyles.title} allowFontScaling={false}>
  ...
  _renderBiometricsPattern = item => {
    ...
    return <View style={itemStyles.row}>
        ...
        <Switch value={isEnabled} onValueChange={this._toggleBiometrics} trackColor={{
        false: '#D6D7DA',
        true: MainTheme.colorSeptenary
  ...
  _renderBBLQrPaymentPattern = item => {
    ...
    return <View style={itemStyles.row}>
        ...
        <Switch value={isEnabled} onValueChange={this._toggleBBLQrPayment} trackColor={{
        false: '#D6D7DA',
        true: MainTheme.colorSeptenary
      }} thumbColor={isEnabled ? MainTheme.colorPrimary : '#F4F3F4'} />
      </View>;
  ...
  _getCurrentBiometricUser = async () => {
    ...
    return currentUser.USER_CODE ? currentUser : biometricState?.userInfo ?? currentUser;
  };
  _toggleBiometrics = async value => {
    ...
    try {
      ...
      if (!sensorInfo?.available || !sensorInfo?.isDeviceSecure) {
        Alert.alert('ไม่สามารถเปิดใช้งาน Biometrics', 'อุปกรณ์นี้ยังไม่รองรับหรือยังไม่ได้ตั้งค่า Biometrics');
        return;
      }
      ...
      if (!username || !password) {
        Alert.alert('ไม่สามารถเปิดใช้งาน Biometrics', 'ไม่พบรหัสผ่านที่บันทึกไว้ กรุณาเข้าสู่ระบบใหม่และเปิดใช้งานอีกครั้ง');
        return;
      }
      const saved = await saveCredentials(username, password);
      if (!saved) {
        Alert.alert('ไม่สามารถเปิดใช้งาน Biometrics', 'ไม่สามารถบันทึกข้อมูลเข้าสู่ระบบสำหรับ Biometrics ได้');
        return;
      }
      ...
      Alert.alert('ไม่สามารถเปิดใช้งาน Biometrics', error?.message || 'เกิดข้อผิดพลาดในการตรวจสอบ Biometrics');
    }
  };
  _toggleBBLQrPayment = async value => {
    await setBBLQrPaymentEnabled(value);
    this.setState({
      bblQrPaymentEnabled: !!value
    });
  };
  _logout = async item => {
  ...
  render() {
    return <ListItems header={this._header} listItems={this._getListItems()} renderItem={this._renderItem} />;
  }
}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    persistBiometricPreference: (value, userInfo) => dispatch(persistBiometricPreference(value, userInfo))
  };
};
