...
const ICamera = ({
  ...
  return <View style={styles.container}>
			<Camera ref={camera} style={StyleSheet.absoluteFill} device={device} isActive={isFocused} photo={Boolean(takePicture)} codeScanner={onBarCodeRead ? codeScanner : undefined} />

			{barcodeFinderVisible ? <BarcodeFinder width={280} height={220} borderColor="#FF4D4F" borderWidth={2} /> : null}
			...
			{!barcodeFinderVisible ? <View style={styles.footer}>
					<View style={styles.sideControl} />
					<TouchableOpacity onPress={handleTakePicture} disabled={working} style={[styles.captureButton, working ? styles.captureButtonDisabled : null]}>
						<Text style={styles.captureButtonText}>{working ? 'กำลังถ่าย...' : 'ถ่าย'}</Text>
					</TouchableOpacity>
					{reverseCamera ? <TouchableOpacity style={styles.sideControl} onPress={() => setCameraPosition(current => current === 'back' ? 'front' : 'back')}>
							<Text style={styles.switchText}>สลับ</Text>
						</TouchableOpacity> : <View style={styles.sideControl} />}
				</View> : <View style={styles.scanHintWrap}>
