...
const DATE_TIME_FORMATS = ['DD/MM/YYYY HH:mm', 'DD/MM/YYYY'];
...
const formatDisplayValue = value => {
  ...
  const parsed = moment(value, DATE_TIME_FORMATS, true);
  if (!parsed.isValid()) {
    return value;
  }
...
class IDateTimePicker extends Component {
  constructor(props) {
    super(props);
  ...
  _parseValue = value => {
    const parsed = value ? moment(value, DATE_TIME_FORMATS, true) : moment();
    return parsed.isValid() ? parsed : moment();
  };

  _setBeginState = async () => {
    const parsed = this._parseValue(this.props.value);
  ...
  render() {
    ...
    const hourItems = this._buildSelectorItems(
      this._generateHourItems(),
      item => item.toString().padStart(2, '0'),
    );
    const minuteItems = this._buildSelectorItems(
      this._generateMinuteItems(),
      item => item.toString().padStart(2, '0'),
    );
          ...
          <View style={styles.overlayCard}>
            ...
            <View style={styles.pickerRow}>
              <View style={styles.pickerColumn}>
                {this._renderSelectField({
                  items: hourItems,
                  selectedKey: this.state.hour,
                  onChange: selectedHour => this._setState('hour', selectedHour),
                  valueText: this.state.hour.toString().padStart(2, '0'),
                })}
              </View>
              ...
              <View style={styles.pickerColumn}>
                {this._renderSelectField({
                  items: minuteItems,
                  selectedKey: this.state.minute,
                  onChange: selectedMinute =>
                    this._setState('minute', selectedMinute),
                  valueText: this.state.minute.toString().padStart(2, '0'),
                })}
              </View>
            </View>
