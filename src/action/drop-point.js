...
import { getDPWareLocationByVANCNF_KEY, getDPWareLocationANDHoByVANCNF_KEY } from '../api/drop-point_wh';
import { lookupErpV3Api, readErpV3Api, updateErpV3Api, parseResDataToJson } from '../api/bPlusApi';
...
import { BPAPUS_FUNCTION_WH_CODE, BPAPUS_FUNCTION_V_CODE, BPAPUS_FUNCTION_BK_CODE, BPAPUS_FUNCTION_DP_CODE } from '../constant/bPlusApi';
export const setInitialState = () => dispatch => {
  dispatch({
    type: types.DROP_POINT_SET_INITIAL_STATE
  });
};
export const clearDropPointList = () => dispatch => {
  return new Promise((resolve, reject) => {
    dispatch({
      type: types.DROP_POINT_CLEAR_LIST_ITEMS
    });
    resolve();
  });
};
export const getDropPointListItems = () => (dispatch, getState) => {
  return new Promise(async (resolve, reject) => {
    ...
    getDPWareLocationANDHoByVANCNF_KEY(product.item.SKU_CODE, userTokens.VANCONFIG).then(v => {
      const {
        ReasonString,
        ResponseCode,
        ResponseData
      ...
      if (ResponseCode == 200) {
        ...
        if (ShowSkuBalance && product.item.SKU_CODE == SKU_CODE && ShowSkuBalance.length > 0) {
          let RESULT = ShowSkuBalance.map(obj => {
            {
              return {
                ...obj,
...
export const searchDropPointList = textInput => async (dispatch, getState) => {
  dispatch({
    type: types.DROP_POINT_SEARCH_LIST_ITEMS
  });
  ...
  await lookupErpV3Api(dataObj1).then(v => {
    ...
    if (ResponseCode == 200) {
      ...
      if (response.Vans0106 && response.RECORD_COUNT && response.RECORD_COUNT > 0) {
        dispatch({
      ...
      dispatch({
        type: types.DROP_POINT_SEARCH_LIST_ITEMS_FAIL
      });
    }
    resolve(v.data);
  }).catch(error => {
