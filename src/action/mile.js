...
export const getMileageLessThanLatestMessage = latestMileage =>
  `เลขไมล์ต้องไม่น้อยกว่า ${normalizeLatestMileage(latestMileage)} (ล่าสุด)`;
...
export const fetchLatestVehicleMileage = () => async dispatch => {
  try {
    ...
    if (!vanCode) {
      dispatch({ type: types.MILE_SET_LATEST_MILEAGE, payload: 0 });
      return 0;
    }
    ...
    console.log('[Mile] fetchLatestVehicleMileage success', {
      vanCode,
      licensePlate,
      latestMileage,
      response,
    ...
    dispatch({ type: types.MILE_SET_LATEST_MILEAGE, payload: latestMileage });
...
const resolveSaveVehicleMileageErrorMessage = error =>
  error?.response?.data?.message ||
  error?.message ||
  'บันทึกเลขไมล์ไม่สำเร็จ';
