...
export const searchProductList =
  (screen, nextPage) => async (dispatch, getState) => {
    ...
    try {
      ...
      const transferSourceWarehouse = isTransferOrder
        ? order.header?.FROM ?? {}
        : {};
      console.log(
        'searchProductList DEBUG: screen=',
        screen,
        'nextPage=',
        nextPage,
      );
      console.log(
        'searchProductList DEBUG: instanceV3 baseURL=',
      ...
      if (!Request.instanceV3.defaults.baseURL && storedWebURL) {
        console.log(
          'searchProductList FIX: re-setting instanceV3 baseURL to',
          storedWebURL,
        );
      ...
      if (!VANCONFIG) {
        console.log(
          'searchProductList ERROR: VANCONFIG is undefined in userToken',
      ...
      const selectedWarehouseKey =
        transferSourceWarehouse.WL_KEY ?? VANCONFIG.VANCNF_WL;
      console.log(
        'searchProductList DEBUG: VANCNF_SKU_LIMIT=',
      ...
      const criteria = {
        ...
        ICDEPT_THAIDESC:
          productCategory.item.ICDEPT_THAIDESC === 'ทั้งหมด'
            ? null
            : productCategory.item.ICDEPT_THAIDESC,
      ...
      let api2 = null;
      const ARPRB_CODE = await getVanArprbCodeApi(
        isTransferOrder ? undefined : customer.item.INFO.AR_KEY,
        VANCONFIG,
      );
      console.log(
        'searchProductList REQUEST_CONTEXT',
        JSON.stringify({
          screen,
          nextPage,
          isTransferOrder,
          skuLimit: VANCONFIG.VANCNF_SKU_LIMIT,
          arprbMode: VANCONFIG.VANCNF_ARPRB_MODE,
          selectedWarehouseKey,
          selectedWarehouseCode: WL_CODE,
      ...
      if (
        ...
        console.log(
          'searchProductList CASE',
          parseInt(VANCONFIG.VANCNF_SKU_LIMIT) == 4
            ? 'SKU_LIMIT_4_DOCUMENT_TRANSFER'
            : 'SKU_LIMIT_3_DOCUMENT_QUOTATION',
        );
          ...
          .then(async v => {
            ...
            if (ResponseCode == 200 && parseInt(RECORD_COUNT) > 0) {
              ...
              for (let i of resultData) {
                ...
                console.log(
                  'searchProductList DOCUMENT_DETAIL_REQUEST',
                ...
                try {
                  ...
                  const { SLDETAIL, DOCINFO, TRANSTKD, RECORD_COUNT } =
                              ...
                              for (
                                ...
                                let temp3 = responseData3.Repack[s];
                                {
                                  await productSearchListV3Api(
                                    ...
                                    let responseData4 = JSON.parse(
                                      l.ResponseData,
                                    );
                                    if (l.ResponseCode == 200) {
                                      if (
                                        responseData4 &&
                                        responseData4.GoodsInfo.length > 0
                                      ) {
                                        if (
                                          responseData4.GoodsInfo[0]
                                            .ARPLU_U_PRC &&
                                          responseData4.GoodsInfo[0]
                                            .ARPLU_U_PRC != ''
                                        ) {
                                          ...
                                          temp2.SKU_KEY = parseFloat(
                                            responseData4.GoodsInfo[0].SKU_KEY,
                                          );
                                          temp2.UTQ_QTY = parseFloat(
                                            responseData4.GoodsInfo[0].UTQ_QTY,
                                          );
                                          temp2.SKU_SKUALT = parseFloat(
                                            responseData4.GoodsInfo[0]
                                              .SKU_SKUALT,
                                          );
                                          temp2.SKU_ICCAT = parseFloat(
                                            responseData4.GoodsInfo[0]
                                              .SKU_ICCAT,
                                          );
                                          temp2.SKU_BRN = parseFloat(
                                            responseData4.GoodsInfo[0].SKU_BRN,
                                          );
                                          temp2.SKU_ICCOLOR = parseFloat(
                                            responseData4.GoodsInfo[0]
                                              .SKU_ICCOLOR,
                                          );
                                          temp2.SKU_ICSIZE = parseFloat(
                                            responseData4.GoodsInfo[0]
                                              .SKU_ICSIZE,
                                          );
                                          temp2.SKU_ICDEPT = parseFloat(
                                            responseData4.GoodsInfo[0]
                                              .SKU_ICDEPT,
                                          );
                                          temp2.UTQ_KEY = parseFloat(
                                            responseData4.GoodsInfo[0].UTQ_KEY,
                                          );
                                          // temp2.ICDEPT_THAIDESC =responseData4.GoodsInfo[0].ICDEPT_THAIDESC;
                                          temp2.good_inVan_qty =
                                            parseFloat(good_inVan_qty);
                                          temp2.SKU_BRN = null;
                        ...
                        await productSearchListV3Api(
                          ...
                          if (k.ResponseCode == 200) {
                            if (
                              ...
                              var clean = Response.filter(
                                (Response, index, self) =>
                                  index ===
                                  self.findIndex(
                                    t =>
                                      t.GOODS_CODE === Response.GOODS_CODE &&
                                      t.UTQ_NAME === Response.UTQ_NAME,
                                  ),
                              );
                              Response = clean.sort(
                                (a, b) => a.SKU_CODE - b.SKU_CODE,
                              ); //.sort((a, b) => a.GOODS_CODE - b.GOODS_CODE);
        ...
        if (screen === 'ProductAddTo') {
          if (parseInt(VANCONFIG.VANCNF_SKU_LIMIT) === 1 && !isTransferOrder) {
            console.log('searchProductList CASE', 'PRODUCT_ADD_TO_SKU_LIMIT_1');
            // console.log("เส้น >>> ", 0);
            ...
            if (isTransferOrder || VANCONFIG.VANCNF_ARPRB_MODE === 1) {
              console.log(
                'searchProductList CASE',
                isTransferOrder
                  ? 'PRODUCT_ADD_TO_TRANSFER'
                  : 'PRODUCT_ADD_TO_ARPRB_MODE_1',
              );
              // console.log("เส้น >>> ", 1);
              api2 = productSearchListByARPLUAPiByCode(
                ARPRB_CODE,
                isTransferOrder ? selectedWarehouseKey : VANCONFIG.VANCNF_WL,
                criteria,
                isTransferOrder && parseInt(VANCONFIG.VANCNF_NOV_SKU_BAL) === 2,
              );
            } else if (VANCONFIG.VANCNF_ARPRB_MODE === 0) {
              console.log(
                'searchProductList CASE',
                'PRODUCT_ADD_TO_ARPRB_MODE_0',
              );
              // console.log("เส้น >>> ", 2);
              api2 = productSearchListByARPLUAPiBykey(
              ...
              console.log(
                'searchProductList CASE',
                'PRODUCT_ADD_TO_FALLBACK_VAN_V3',
              );
              // console.log("เส้น >>> ", 3);
              api2 = productSearchListByVanV3Api(criteria);
            }
          ...
          console.log('searchProductList CASE', 'STOCK_SEARCH');
          {
            // console.log("เส้น >>> ", 5);
            let _wl_codes = [];
            let dataObj1 = {
              ...
              'BPAPUS-PARAM': '',
              'BPAPUS-FILTER':
                "and VANCNF_KEY = '" + VANCONFIG.VANCNF_KEY + "'",
              'BPAPUS-ORDERBY': '',
              'BPAPUS-OFFSET': '0',
              'BPAPUS-FETCH': '0',
            };
            ...
            if (parseInt(VANCONFIG.VANCNF_SKU_LIMIT) === 1) {
              console.log('searchProductList CASE', 'STOCK_SEARCH_SKU_LIMIT_1');
              ...
              console.log(
                'searchProductList CASE',
                'STOCK_SEARCH_MULTI_WAREHOUSE',
              );
              // console.log("เส้น >>> ", 0.2);
              api2 = productSearchListByARPLUAPiByCodeAndWHIS1(
                ARPRB_CODE,
                _wl_codes,
                criteria,
              );
        ...
        console.log(
          'searchProductList PRODUCT_LIST_API',
          JSON.stringify({
            screen,
            isTransferOrder,
            skuLimit: VANCONFIG.VANCNF_SKU_LIMIT,
        ...
        await api2
          .then(async v => {
            ...
            let responseData = JSON.parse(ResponseData);
            console.log(
              '[STEP 1] ดึงรายการเบื้องต้น ResponseCode=',
              ResponseCode,
              'RECORD_COUNT=',
              responseData?.RECORD_COUNT,
              'Vans0105=',
              responseData?.Vans0105?.length,
              'Sp000221=',
              responseData?.Sp000221?.length,
            );
            if (ResponseCode == 200) {
              ...
              if (
                ...
                for (let i in additionalData) {
                  good_inVan_qty = 0;
                  await getWareLocationStockBalance(
                    additionalData[i].GOODS_CODE,
                    VANCONFIG,
                    isTransferOrder ? selectedWarehouseKey : undefined,
                  ).then(v => {
                    const { ReasonString, ResponseCode, ResponseData } = v;
                    let responseData = JSON.parse(ResponseData);
                    console.log(
                      '[STEP 2] เช็ค stock คลังต้นทาง i=',
                      i,
                      'GOODS_CODE=',
                      additionalData[i].GOODS_CODE,
                      'WL_CODE=',
                      WL_CODE,
                      'ShowSkuBalance=',
                      JSON.stringify(responseData?.ShowSkuBalance),
                    );
                    if (ResponseCode == 200) {
                  ...
                  await productSearchListV3Api(
                    ...
                    .then(k => {
                      ...
                      let responseData2 = JSON.parse(k.ResponseData);
                      console.log(
                        '[STEP 3] ดึง Detail+ราคา i=',
                        i,
                        'GOODS_CODE=',
                        additionalData[i].GOODS_CODE,
                        'GoodsInfo.length=',
                        responseData2?.GoodsInfo?.length,
                        'ARPLU_U_PRC=',
                        responseData2?.GoodsInfo?.[0]?.ARPLU_U_PRC,
                        'good_inVan_qty=',
                        good_inVan_qty,
                      ...
                      if (k.ResponseCode == 200) {
                        ...
                        if (
                          ...
                          if (
                            ...
                            const allowNegativeTransferQty =
                              parseInt(VANCONFIG.VANCNF_NOV_SKU_BAL) === 2;
                            const shouldIncludeItem = isTransferOrder
                              ? allowNegativeTransferQty ||
                                parseFloat(good_inVan_qty) > 0
                              : parseInt(VANCONFIG.VANCNF_SKU_LIMIT) == 1 ||
                                parseInt(VANCONFIG.VANCNF_SKU_LIMIT) == 3 ||
                                parseInt(VANCONFIG.VANCNF_SKU_LIMIT) == 4 ||
                                (parseInt(VANCONFIG.VANCNF_SKU_LIMIT) == 2 &&
                                  parseFloat(good_inVan_qty) >= 0);
                            console.log(
                              '[STEP 3.1] shouldIncludeItem=',
                              shouldIncludeItem,
                              'isTransfer=',
                              isTransferOrder,
                              'allowNeg=',
                          ...
                          console.log(
                            'yyyyyy 1 ',
                            types.PRODUCT_SEARCH_LIST_SUCCESS,
                          );
                          dispatch({
                            type: types.PRODUCT_SEARCH_LIST_SUCCESS,
                            payload: [],
      ...
      if (Response && Response.length > 0) {
        console.log(
          'searchProductList DEBUG: Response count=',
          Response.length,
        );
        dispatch(
          setCriteria({
      ...
      console.log(
        'searchProductList CRITICAL ERROR:',
        searchError && searchError.message ? searchError.message : searchError,
      );
      console.log(
        'searchProductList CRITICAL ERROR stack:',
        searchError && searchError.stack,
      );
      dispatch({ type: types.PRODUCT_SEARCH_LIST_FAIL });
    }
  };

export const searchProductList_bk =
  (screen, nextPage) => async (dispatch, getState) => {
    ...
    if (VANCONFIG.VANCNF_SKU_LIMIT == 3 || VANCONFIG.VANCNF_SKU_LIMIT == 4) {
      ...
      console.log(
        'Bazzz  VANCONFIG VANCNF_SKU_LIMIT==>',
        VANCONFIG.VANCNF_SKU_LIMIT,
      );
      console.log(
        'Bazzz  VANCONFIG VANCNF_SKU_LIMIT==>',
      ...
      await lookupErpV3Api(RequestBody)
        .then(async v => {
          ...
          const { ReasonString, ResponseCode, ResponseData } = v.data;
          const { [wsfunction]: resultData, RECORD_COUNT } = parseResDataToJson(
            v.data,
          );
              ...
              await updateErpV3Api(ReqBody)
                .then(async res => {
                  ...
                  if (VANCONFIG.VANCNF_SKU_LIMIT == '3') {
                    if (
                      parseInt(SLDETAIL.SLMN_KEY) !==
                      parseInt(VANCONFIG.VANCNF_SLMN)
                    ) {
                      return;
                    }
                  }
                  ...
                  if (VANCONFIG.VANCNF_SKU_LIMIT == '4') {
                    if (
                      parseInt(TRANSTKD[0].TRD_TO_WL) !==
                      parseInt(VANCONFIG.VANCNF_WL)
                    ) {
                      return;
                    }
                  }
                  ...
                  console.log(
                    '(parseInt(TRANSTKD[0].TRD_TO_WL) !== parseInt(VANCONFIG.VANCNF_WL))',
                    parseInt(TRANSTKD[0].TRD_TO_WL) !==
                      parseInt(VANCONFIG.VANCNF_WL),
                  );
                  console.log('TRANSTKD.TRD_TO_WL', TRANSTKD[0].TRD_TO_WL);
                    ...
                    for (let j of TRANSTKD) {
                      ...
                      if (
                        (VANCONFIG.VANCNF_SKU_LIMIT == 4 &&
                          j.TRD_TO_WL == VANCONFIG.VANCNF_WL) ||
                        (VANCONFIG.VANCNF_SKU_LIMIT == 3 &&
                          j.TRD_WL == VANCONFIG.VANCNF_WL)
                      )
                        console.log('dataObj2', 2);
              ...
              for (let i in additionalData) {
                ...
                await productSearchListV3Api(
                  ...
                  .then(k => {
                    ...
                    if (k.ResponseCode == 200) {
                      console.log(
                        'productSearchListV3Api responseData2.GoodsInfo.length ==== ',
                        responseData2.GoodsInfo.length,
                      );
...
export const searchProductByGoodsCode = () => (dispatch, getState) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (isTransferOrder && !selectedWarehouseCode) {
      ...
      await lookupErpV3Api(warehouseRequest)
        .then(v => {
          const { ResponseData, ResponseCode, ReasonString } = v.data;
          if (ResponseCode == 200) {
            let responseData = JSON.parse(ResponseData);
            selectedWarehouseCode = responseData.Wh000220
              ? responseData.Wh000220[0].WL_CODE
              : null;
          } else {
            console.log('ERROR lookupErpV3Api', ReasonString);
        ...
        if (parseInt(responseData.RECORD_COUNT) > 0) {
          ...
          if (isTransferOrder && selectedWarehouseCode) {
            await getWareLocationStockBalance(
              ...
              .then(stockResponse => {
                const { ResponseCode: stockCode, ResponseData: stockData } =
                  stockResponse;
                if (stockCode == 200) {
              ...
              .catch(error => {
                console.log('getWareLocationStockBalance ERROR', error);
              });
          }
          ...
          dispatch({
            ...
            payload: {
              ...
              item: {
                ...
                good_inVan_qty: isTransferOrder
                  ? goodInSourceQty
                  : responseData.GoodsInfo[0].good_inVan_qty,
              },
            },
          });
          resolve(v);
    ...
    if (order.header.AR_ORDER_TYPE == 'ขายสินค้า') {
      ...
      for (let i in data.ITEMS) {
        let newObj = {
          KEY: '',
          TRD_DSC_KEYIN:
            data.ITEMS[i].TRD_DSC_KEYIN + '*' + data.ITEMS[i].TRD_QTY + 'B',
      ...
      order.header.AR_ORDER_TYPE == 'จองสินค้า' ||
      order.header.AR_ORDER_TYPE == 'ใบเสนอราคา'
    ) {
      ...
      processOrderItemV3Api(dataObj)
        .then(v => {
          ...
          if (ResponseCode == 200) {
            ...
            //dispatch({type: types.ORDER_CREATE_FAIL, payload: ReasonString});
            console.log(
              'จองสินค้า ERROR processOrderItemV3Api',
              ResponseData + ReasonString,
            );
