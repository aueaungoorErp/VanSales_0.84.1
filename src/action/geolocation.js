...
const requestLocationPermission = async () => {
  ...
  const grantedPermission = fineResult === PermissionsAndroid.RESULTS.GRANTED || coarseResult === PermissionsAndroid.RESULTS.GRANTED;
  const blockedPermission = fineResult === PermissionsAndroid.RESULTS.NEVER_ASK_AGAIN || coarseResult === PermissionsAndroid.RESULTS.NEVER_ASK_AGAIN;
  return {
...
export const setInitialState = () => dispatch => {
  return new Promise((resolve, reject) => {
    dispatch({
      type: types.GEOLOCATION_SET_INITIAL_STATE
    });
    resolve();
  });
};
export const getCurrentPosition = () => dispatch => {
  return new Promise((resolve, reject) => {
    dispatch({
      type: types.GEOLOCATION_GET_CURRENT_POSITION
    });
    ...
    const run = async () => {
      ...
      if (!permissionState.granted) {
        handleFailure(permissionState.blocked ? LOCATION_ERROR_MESSAGES.permissionBlocked : LOCATION_ERROR_MESSAGES.permissionDenied);
        return;
      }
...
export const setMessage = message => dispatch => {
  return new Promise((resolve, reject) => {
    dispatch({
      type: types.GEOLOCATION_SET_MESSAGE,
      payload: message
    });
    resolve();
  });
