...
const getQuotationExpiryMoment = header => {
  const expiryMoment = header?.VDI_EXP_DATE
    ? moment(header.VDI_EXP_DATE, 'YYYYMMDD', true)
    : null;
...
const getQuotationShipMoment = header => {
  const shipMoment = header?.VDI_SHIP_DATE
    ? moment(header.VDI_SHIP_DATE, 'YYYYMMDD', true)
    : null;
...
export const processOrderSale =
  (data, order, vanConfig) => (dispatch, getState) => {
      ...
      if (isNegative) {
        if (order.header.AR_ORDER_TYPE === 'ขายสินค้า') {
          dispatch({
            ...
            payload:
              'สินค้าในคลังมีไม่พอ หรือ ไม่สามารถขายสินค้าจำนวนติดลบได้ :' +
              good_inVan_qty,
          });
        ...
        if (
          ...
          console.log('quotation processOrderSale expiry debug', {
            processDetail: data.PROCESS_DETAIL,
            VDI_DATE: order.header?.VDI_DATE,
            VDI_SHIP_DATE: order.header?.VDI_SHIP_DATE,
            VDI_EXP_DATE: order.header?.VDI_EXP_DATE,
            quotationShipMoment: quotationShipMoment.format('YYYYMMDD'),
            quotationExpiryMoment: quotationExpiryMoment.format('YYYYMMDD'),
          });
          param = {
            ErpUpdFunc: [
              {
...
export const createOrderSaleV3 =
  (data, V3GUID, vanConfig, paymentType) => (dispatch, getState) => {
    return new Promise(async (resolve, reject) => {
        ...
        ErpUpdFunc: [
          {
            ImpTrhHeader: {
              TRH_ARPRB:
                vanConfig.VANCNF_ARPRB /* ตารางราคาที่ใช้อ้างอิงบิลนี้*/,
              DI_DATE: moment(order.header.VDI_DATE).format('YYYYMMDD'),
              DI_REF: '<เลขถัดไป>',
              DT_DOCCODE: VANCNF_BOOK_DT,
              DT_PROPERTIES: DT_PROPERTIES,
          ...
          if (ResponseCode == 200) {
            ...
            if (order.header.AR_ORDER_TYPE === 'ขายสินค้า') {
              dataObj00 = {
                ...
                'BPAPUS-PARAM':
                  '{"DI_KEY": "' + JSON.parse(ResponseData).DI_KEY + '"}',
                'BPAPUS-FILTER': '',
                'BPAPUS-ORDERBY': '',
                'BPAPUS-OFFSET': '0',
                'BPAPUS-FETCH': '0',
            ...
            TRANSTKD.then(x => {
              const { ResponseData, ResponseCode, ReasonString } = x.data;
...
export const orderReservV3 =
  (data, V3GUID, vanConfig) => (dispatch, getState) => {
    return new Promise(async (resolve, reject) => {
      dispatch({ type: types.ORDER_CREATE });
      const order = getState().order;
      const customer = getState().customer;
      ...
      for (let i in data.ITEMS) {
        ...
        let newObj = {
          ...
          TRD_OPTION: data.ITEMS[i].TRD_OPTION,
          TRD_KEYIN: data.ITEMS[i].TRD_KEYIN,
          SKU_NAME: data.ITEMS[i].TRD_SH_NAME
      ...
      for (let i in data.ITEMS) {
        let newObj = {
          ...
          TRD_DSC_KEYIN: data.ITEMS[i].VTRD_U_DISC_TEXT
            ...
            : data.ITEMS[i].TRD_DSC_KEYIN +
              '*' +
              (data.ITEMS[i].VTRD_QTY
...
export const orderReturn =
  (data, type, vanConfig, V3GUID) => (dispatch, getState) => {
    return new Promise(async (resolve, reject) => {
      const order = getState().order;
      const customer = getState().customer;
      dispatch({ type: types.ORDER_CREATE });
...
export const getProductListItemsFromLastBillByArCode =
  (V3GUID, vancnf_machine, goodsCodes) => (dispatch, getState) => {
    return new Promise(async (resolve, reject) => {
      const customer = getState().customer;
      const order = getState().order;
      ...
      console.log(
        'order.header.AR_ORDER_TYPE VANCONFIG ',
          ...
          .then(async v => {
            ...
            if (
              ...
              await lookupErpV3Api(dataObj2).then(g => {
                const { ResponseData, ResponseCode, ReasonString } = g.data;
              ...
              TRANSTKD.then(v => {
                ...
                if (ResponseCode == 200) {
                  ...
                  if (order.header.AR_ORDER_TYPE === 'ขายสินค้า') {
                    order.orderProductSummary.DIS_BILL_1 =
                      responseData.ARDETAIL.ARD_TDSC_KEYIN.split(',')[0];
                    order.orderProductSummary.DIS_BILL_2 =
                      responseData.ARDETAIL.ARD_TDSC_KEYIN.split(',')[1];
                    ...
                    order.orderProductSummary.DIS_BILL_1 =
                      responseData.AROE.AROE_TDSC_KEYIN.split(',')[0];
                    order.orderProductSummary.DIS_BILL_2 =
                      responseData.AROE.AROE_TDSC_KEYIN.split(',')[1];
                    order.orderProductSummary.totalQty = 1;
                  }
                  ...
                  console.log(
                    'responseData1.Oe002304[0].DI_REF ',
                    responseData.TRANSTKD,
                  );
                  ...
                  for (let obj of filteredTranstkd) {
                    good_inVan = getWareLocationStockBalance(
                      obj.GOODS_CODE,
                      VANCONFIG,
                    ).then(v2 => {
                      const { ReasonString, ResponseCode, ResponseData } = v2;
...
export const createQuotation =
  (data, V3GUID, vanConfig) => (dispatch, getState) => {
    return new Promise(async (resolve, reject) => {
      ...
      const quotationExpiryMoment = getQuotationExpiryMoment(order.header);
      const quotationShipMoment = getQuotationShipMoment(order.header);
      console.log('quotation createQuotation expiry debug', {
        VDI_DATE: order.header?.VDI_DATE,
        VDI_SHIP_DATE: order.header?.VDI_SHIP_DATE,
        VDI_EXP_DATE: order.header?.VDI_EXP_DATE,
        quotationShipMoment: quotationShipMoment.format('YYYYMMDD'),
      ...
      console.log(
        'createQuotation header payload',
        JSON.stringify(param.ErpUpdFunc?.[0]?.ImpTrhHeader),
      );
