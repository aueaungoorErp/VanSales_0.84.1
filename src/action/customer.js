...
const normalizeCustomerDetail = async (detailItem, VANCONFIG) => {
  ...
  const ARPRB_KEY = await getCustArprbKEYApi(parseFloat(detailItem.AR_KEY), VANCONFIG);
  temp.AR_KEY = parseFloat(detailItem.AR_KEY);
  temp.AR_ARCAT = detailItem.AR_ARCAT;
  temp.ADDB_KEY = parseFloat(detailItem.ADDB_KEY);
  temp.ADDB_GPS_LAT_S = detailItem.ADDB_GPS_LAT_S && !isNaN(detailItem.ADDB_GPS_LAT_S) ? parseFloat(detailItem.ADDB_GPS_LAT_S) : null;
  temp.ADDB_GPS_LONG_S = detailItem.ADDB_GPS_LONG_S && !isNaN(detailItem.ADDB_GPS_LONG_S) ? parseFloat(detailItem.ADDB_GPS_LONG_S) : null;
...
export const setInitialState = () => dispatch => {
  dispatch({
    type: types.CUSTOMER_SET_INITIAL_STATE
  });
};
export const clearCustomerList = () => dispatch => {
  return new Promise((resolve, reject) => {
    dispatch({
      type: types.CUSTOMER_CLEAR_LIST
    });
    resolve();
  });
};
export const setCriteria = criteria => dispatch => {
  return new Promise((resolve, reject) => {
    dispatch({
      type: types.CUSTOMER_SET_CRITERIA,
      payload: criteria
    });
    resolve();
  });
...
export const setKeyword = criteria => dispatch => {
  dispatch({
    type: types.CUSTOMER_SET_KEYWORD,
    payload: criteria
  });
};
export const findCustomerById = id => dispatch => {
  return new Promise(async (resolve, reject) => {
    ...
    const bodyRequest = {
      ...
      //SHOWARSUMMARY 39 แสดงสรุปยอดลูกหนี้
    ...
    executiveV3Api(bodyRequest).then(async v => {
      const {
        ReasonString,
        ResponseCode,
        ResponseData
...
export const searchCustomerList = (nextPage, fetchCustomerDetailsBatch) => async (dispatch, getState) => {
  const startedAt = Date.now();
  dispatch({
    type: types.CUSTOMER_SEARCH_LIST
  });
  ...
  const criteria = {
    ...
    OFFSET: nextPage ? (customer.criteria.OFFSET - 1) * customer.criteria.LIMIT : (1 - 1) * customer.criteria.LIMIT,
    LIMIT: customer.criteria.LIMIT
  };
  const routeFetchStartedAt = Date.now();
  return await customerSearchArLineListV3Api(criteria).then(async v => {
    const {
      ReasonString,
      ResponseCode,
      ResponseData
    ...
    if (ResponseCode == 200) {
      ...
      if (responseData && additionalData && parseInt(RECORD_COUNT) > 0) {
        ...
        if (typeof fetchCustomerDetailsBatch === 'function') {
          const batchArCodes = additionalData.map(item => item?.AR_CODE).filter(item => item !== undefined && item !== null && item !== '');
          try {
            ...
            const detailItems = Array.isArray(batchResponseData?.Ar000131) ? batchResponseData.Ar000131 : [];
            const detailMap = new Map(detailItems.map(item => [String(item.AR_CODE), item]));
            for (const routeItem of additionalData) {
              const matchedDetail = detailMap.get(String(routeItem.AR_CODE));
              if (!matchedDetail) {
                continue;
              }
              const normalizedDetail = await normalizeCustomerDetail(matchedDetail);
              const hasExisting = Response.some(item => item.AR_KEY == normalizedDetail.AR_KEY);
              if (!hasExisting) {
                Response.push({
                  ...routeItem,
                  ...normalizedDetail
          ...
          for (let i in additionalData) {
            await customerSearchListV3Api(additionalData[i].AR_CODE, criteria.ARCAT_KEY).then(async v => {
              found = false;
              const {
                ReasonString,
                ResponseCode,
              ...
              if (ResponseCode == 200) {
                ...
                if (responseData && Ar000131 && parseInt(RECORD_COUNT) > 0) {
                  const normalizedDetail = await normalizeCustomerDetail(Ar000131[0]);
        ...
        const rawItemCount = Array.isArray(additionalData) ? additionalData.length : 0;
...
export const searchCustomerRoutePageOnly = nextPage => async (dispatch, getState) => {
  const startedAt = Date.now();
  let customer = await getState().customer;
  const customerType = await getState().customerType;
  const userToken = await getUserToken();
...
export const appendCustomerRouteBatchDetails = (routeItems, fetchCustomerDetailsBatch, hasMore = false) => async (dispatch, getState) => {
  const startedAt = Date.now();
  const userToken = await getUserToken();
  const VANCONFIG = userToken?.VANCONFIG ?? {};
  ...
  try {
    ...
    for (const routeItem of additionalData) {
      ...
      const normalizedDetail = await normalizeCustomerDetail(matchedDetail, VANCONFIG);
      const hasExisting = Response.some(item => item.AR_KEY == normalizedDetail.AR_KEY);
      if (!hasExisting) {
        Response.push({
          ...routeItem,
...
export const searchCustomerNextDestination = () => (dispatch, getState) => {
  ...
  //const result =
  dispatch(getCustomerNextDestination('')).then(v => {
    let responseData = v;
    //if (ResponseCode == 200)
    {
...
export const getCustomerNextDestination = AR_CODE => dispatch => {
  return new Promise(async (resolve, reject) => {
    ...
    await getARLV3Api(bodyRequest).then(v => {
      // const {RESULT_DATA, STATUS, ERROR_MESSAGES} = v;
      const {
        ReasonString,
        ResponseCode,
...
export const _getCustomerNextDestination = () => dispatch => {
  return new Promise((resolve, reject) => {
    ...
    customerSearchListApi(criteria).then(v => {
      const {
        RESULT_DATA,
        STATUS,
        ERROR_MESSAGES
...
export const setCustomerInfo = data => dispatch => {
  dispatch({
    type: types.CUSTOMER_SET_ITEM_INFO,
    payload: data
  });
};
...
export const setCustomerCusAddb = data => dispatch => {
  dispatch({
    type: types.CUSTOMER_SET_ITEM_CUS_ADDB,
    payload: data
  });
};
export const setCustomerLstVisitDoc = data => dispatch => {
  dispatch({
    type: types.CUSTOMER_SET_ITEM_LST_VISIT_DOC,
    payload: data
  });
};
export const setCustomerLstBillDoc = data => dispatch => {
  dispatch({
    type: types.CUSTOMER_SET_ITEM_LST_BILL_DOC,
    payload: data
  });
};
export const setCustomerCusPayInf = data => dispatch => {
  dispatch({
    type: types.CUSTOMER_SET_ITEM_CUS_PAY_INF,
    payload: data
  });
};
export const clearCustomerTempCus = () => dispatch => {
  return new Promise((resolve, reject) => {
    dispatch({
      type: types.CUSTOMER_CLEAR_ITEM_TEMP_CUS,
      payload: data
    });
    resolve();
  });
...
export const setCustomerTempCus = data => dispatch => {
  return new Promise((resolve, reject) => {
    dispatch({
      type: types.CUSTOMER_SET_ITEM_TEMP_CUS,
      payload: data
    });
    resolve();
  });
  ...
  return new Promise(async (resolve, reject) => {
    ...
    getARL_KEY99(bodyRequest99).then(v => {
      ...
      if (ResponseCode == '200' && parseInt(RECORD_COUNT) > 0) {
        ...
        NewArFileV3Api(bodyRequest).then(v => {
          const {
            ReasonString,
            ResponseCode,
            ResponseData
...
export const searchCustomerNearBy = criteria => dispatch => {
  return new Promise((resolve, reject) => {
    dispatch({
      type: types.CUSTOMER_SEARCH_NEAR_BY
    });
    searchCustomerNearByApi(criteria).then(v => {
      const {
        RESULT_DATA,
        STATUS,
        ERROR_MESSAGES
...
export const closeCustomerAccount = reason => (dispatch, getState) => {
  return new Promise((resolve, reject) => {
    ...
    closeCustomerAccountApi(request).then(v => {
      const {
        STATUS,
        ERROR_MESSAGES
      } = v;
...
export const _getARCustomerLine = () => dispatch => {
  return new Promise((resolve, reject) => {
    dispatch({
      type: types.CUSTOMER_GET_AR_LINE
    });
    const criteria = qs.stringify({
      OFFSET: 0,
      LIMIT: 13
...
export const getArPricetab = () => dispatch => {
  return new Promise(async (resolve, reject) => {
    ...
    await getArPricetabApi(bodyRequest).then(v => {
      const {
        ReasonString,
        ResponseCode,
        ResponseData
