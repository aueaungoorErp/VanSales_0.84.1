...
import { getVanConfigV3Api, lookupErpV3Api, readErpV3Api, systemCheckApi2, unRegisterApi } from '../api/setting';
import { getLoginInfo, getSettingConfig } from '../utils/Token';
import { getCredentials, getSavedUsername } from '../services/SecureCredentials';
export const systemCheck = data => async dispatch => {
  ...
  const finalUserCode = USER_CODE ?? savedSetting?.USER_CODE ?? savedUsername ?? credentials?.username ?? '';
  const finalPassword = USER_PASSWORD ?? savedSetting?.USER_PASSWORD ?? credentials?.password ?? '';
  return await systemCheckApi2(baseUrl, vanCNFMachine, finalUserCode, finalPassword);
};
...
export const getOneTimeGUID = () => async dispatch => {
  return new Promise(async (resolve, reject) => {
    ...
    Request.instanceV3.post('/DevUsers', bodyRequest).then(async v => {
      const {
        ResponseData,
        ResponseCode,
        ReasonString
      ...
      if (ResponseCode == 200) {
        ...
        await Request.instanceV3.post('/DevUsers', bodyRequest).then(v => {
          const {
            ResponseData,
            ResponseCode,
            ReasonString
...
export const readSlCatBySlKeyV3 = () => dispatch => {
  return new Promise(async (resolve, reject) => {
    await readErpV3Api(data).then(v => {
      resolve(v);
    }).catch(err => {
      reject(err.message);
    });
...
export const readCompanyInfoV3 = (GUID, CMPNY_CODE) => dispatch => {
  return new Promise(async (resolve, reject) => {
    ...
    await readErpV3Api(bodyRequest).then(v => {
      resolve(v);
    }).catch(err => {
      reject(err.message);
    });
