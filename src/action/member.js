...
import { FIND_MEMBER_NAME, GET_UPDATE_MEMBER, CREATE_NEWMEMBER } from '../constant/bPlusApi';
export const updateMemberV3Api = async criteria => {
  const LoginGUID = await getLoginGuID();
  const {
    VANCONFIG
  } = await getUserToken();
  const MB_KEY = typeof criteria.MB_KEY == 'object' ? '' : criteria.MB_KEY ? "and MB_KEY = '" + criteria.MB_KEY + "'" : '';
  const MB_NAME = typeof criteria.MB_NAME == 'object' ? '' : criteria.MB_NAME ? "and MB_NAME = '" + criteria.MB_NAME + "'" : '';
  const bodyRequest = {
