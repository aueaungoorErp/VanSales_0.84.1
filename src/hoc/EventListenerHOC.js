...
const EventListenerHOC = Component => {
  return class App extends Component {
    ...
    componentDidMount() {
      ...
      this.printerStateSubscription = DeviceEventEmitter.addListener(
        'printerState',
        event => {
          this._printerStateChange(event);
        },
      );

      this.bluetoothStateSubscription = DeviceEventEmitter.addListener(
        'bluetoothState',
        event => {
          let { state } = event;
    ...
    async _handleAppStateChange(nextAppState) {
      if (!this.done) {
        ...
        const routeName =
          navState && navState.routes
            ? navState.routes[navState.index].name
            : null;
        if (routeName !== 'Login' && routeName !== 'Splash') {
          if (
            this.props.appState.state.match(/inactive|background/) &&
            nextAppState === 'active'
          ) {
            this.done = true;
            ...
            const timeSinceLastAccess =
              moment().valueOf() - (Number(accessTimeToken) || 0);
            // Only force re-auth via biometrics if the app has been in the
